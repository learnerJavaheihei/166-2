 ...
 * @author pchayka
 */
public final class FourSepulchersGatekeeperInstance extends NpcInstance {
	...
	private synchronized void tryEntry(Player player, boolean unstable)
	{
		...
		if(party != null) {
			...
			for (Player member : party.getPartyMembers()) {
				...
				if (member.isDead() || !member.isInRange(player, 700) || member.getLevel() < 80) { // TODO: Пересмотреть сообщение.
					showChatWindow(player, HTML_FILE_PATH + getNpcId() + "-LV.htm", false);
					return;
				}
			}
		} else if(!player.isGM()) {
		...
		showChatWindow(player, HTML_FILE_PATH + getNpcId() + "-OK.htm", false);

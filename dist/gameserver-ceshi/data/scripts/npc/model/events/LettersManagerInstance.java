...
import l2s.gameserver.data.xml.holder.SkillHolder;//增加活動輔助
import l2s.gameserver.model.Player;
import l2s.gameserver.model.Skill;//增加活動輔助
import l2s.gameserver.model.instances.NpcInstance;
import l2s.gameserver.skills.SkillEntry;//增加活動輔助
import l2s.gameserver.skills.SkillEntryType;//增加活動輔助
...
public class LettersManagerInstance extends NpcInstance
{
	...
	private static final int LINEAGE_II_CHEST = 29986;	// Lineage II Chest 修改正確ID
	private static final int CHRONICLE_CHEST = 29988;	// Chronicle Chest 修改正確ID
	private static final int MEMORIES_CHEST = 29987;	// Memories Chest 修改正確ID
	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		else if(ask == -2000)
		{
			...
			if(!ItemFunctions.deleteItem(player, letterId, 10))
			{
				showChatWindow(player, "events/in_the_pursuit_of_letters/" + getNpcId() + "-no_have_letters.htm", false);
				return;
			}
			Skill skill = SkillHolder.getInstance().getSkill(55716, 1);
			player.getAI().Cast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, skill), player, false, true);
		}
		//--增加活動輔助
		else

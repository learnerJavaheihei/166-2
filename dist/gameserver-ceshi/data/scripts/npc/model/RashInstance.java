...
import instances.BalthusKnightZaken;//添加札肯副本
...
{
	private static final int RUSHS_SUPPLIES_ANTHARAS = 90999;	// 雷修補給品-安塔瑞斯
	private static final int RUSHS_SUPPLIES_BAIUM = 91000;	// 雷修補給品-巴溫
	private static final int RUSHS_SUPPLIES_ZAKEN = 91007;	// 雷修補給品-札肯
	private static final int HIGH_GRADE_XP_SCROLL_TICKET = 90947;	// 經驗值卷軸交換券-上級
	private static final int MAGICAL_TABLET = 90045;	// 魔力字板
	private static final int DAMAGED_ANTHARAS_EARRING = 90993;	// 受損的安塔瑞斯耳環
	private static final int DAMAGED_BAIUM_RING = 90994;	// 受損的巴溫戒指
	private static final int DAMAGED_ZAKEN_EARRING = 91006;	// 受損的札肯耳環
	private static final int ANTHARAS_EARRING = 90992;	// 安塔瑞斯耳環
	private static final int BAIUM_RING = 49580;	// 巴溫戒指
	private static final int ZAKEN_EARRING = 90763;	// 札肯耳環
	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		else if(ask == -9001)
		{
			...
			if(reply == 1)
			{
				BalthusKnightZaken balthusKnightBaium = (BalthusKnightZaken) reflection;
				if(balthusKnightBaium.isRewardReceived(player))
				{
					showChatWindow(player, "default/" + getNpcId() + "-already_received.htm", false);
					return;
			...
			else if(reply == 2)//購買物品 這裡需要你自己找頁面處理
			{
				MultiSellHolder.getInstance().SeparateAndSend(3171602, player, 0);
			}
			else if(reply == 3)//交換受損的札肯耳環
			{
				ItemInstance ring = player.getInventory().getItemByItemId(DAMAGED_ZAKEN_EARRING);
				if(ring == null || ring.getEnchantLevel() < 10)
				{
					showChatWindow(player, "default/" + getNpcId() + "-not_have_zaken_earring.htm", false);
					return;
				}
				...
				ItemFunctions.addItem(player, ZAKEN_EARRING, 1, true);
			}
			else if(reply == 4)//離開這裡
			{
				player.teleToLocation(getReflection().getReturnLoc(), ReflectionManager.MAIN);
			}
		}

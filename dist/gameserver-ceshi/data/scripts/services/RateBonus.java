...
public class RateBonus
{
	...
	public void list(Player player, NpcInstance npc, String[] param)
	{
		...
		for(PremiumAccountTemplate premiumAccount : PremiumAccountHolder.getInstance().getPremiumAccounts())
		{
			...
			{
				List<ItemData> feeItems = premiumAccount.getFeeItems(delay);
				if(!feeItems.isEmpty())
				{
				...
				if(delay > 0)
				{
					...
					{
						if(player.isLangRus())
							add.append(hours).append("時。");
						else
							add.append(hours).append("时。");
					}
				}
				else
					add.append(player.isLangRus() ? "無限" : "无限");
	...
	public void get(Player player, NpcInstance npc, String[] param)
	{
		...
		if(!feeItems.isEmpty())
		{
			...
			{
				if(feeItems.size() == 1 && feeItems.get(0).getId() == ItemTemplate.ITEM_ID_ADENA)

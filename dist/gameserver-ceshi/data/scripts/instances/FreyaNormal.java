...
{
	private static final int FreyaThrone = 29177;//芙蕾雅 冰雪女王 覺醒
	private static final int FreyaStandNormal = 29179;//芙蕾雅 冰雪女王 覺醒
	private static final int IceKnightNormal = 18855; //冰宮騎士 覺醒
	private static final int IceKnightLeaderNormal = 25699;//格拉奇恩斯 冰宮騎士團長 覺醒
	private static final int IceCastleBreath = 18854;//冰宮的氣息 覺醒
	private static final int Glacier = 18853; // 冰結石 覺醒
	private static final int IceCastleController = 18932; // a, 覺醒
	private static final int Sirra = 32762;//希露 覺醒
	private static final int Jinia = 18850;//吉妮亞 覺醒
	private static final int Kegor = 18851;//凱高爾 覺醒
	...
	{
		for(int triggerId : _eventTriggers)
		{
			if(active)
				addEventTrigger(triggerId);
			else
				removeEventTrigger(triggerId);
		}
	}

	private class StartNormalFreya implements Runnable
	{
		@Override
		public void run() 
		{
			...
			{
				QuestState qs = player.getQuestState(10286);//與希露再次見面 覺醒任務
				if(qs != null && qs.getCond() == 5)
					qs.setCond(6);
				//player.startScenePlayer(SceneMovie.FREYA_OPENING);
			}
			ThreadPoolManager.getInstance().schedule(new PreStage(), 3000L); // 53.5sec for movie
		}
	}

	private class PreStage implements Runnable
	{
		@Override
		public void run() 
		{
			...
			//spawning few guards
			for(int i = 0; i < 10; i++)
				addSpawnWithoutRespawn(IceKnightNormal, Territory.getRandomLoc(centralRoom, getGeoIndex(),false), 0);
	...
	private class GuardSpawnTask implements Runnable
	{
		...
		public void run() 
		{
			...
			for(int i = 0; i < Rnd.get(_breathMin, _breathMax); i++)
				addSpawnWithoutRespawn(IceCastleBreath, Territory.getRandomLoc(centralRoom, getGeoIndex(),false), 0);
			if(Rnd.chance(60))
				for(int i = 0; i < Rnd.get(1, 3); i++)
					addSpawnWithoutRespawn(Glacier, Territory.getRandomLoc(centralRoom, getGeoIndex(),false), 0);
		}
	}

	private class PreSecondStage implements Runnable
	{
		@Override
		public void run() 
		{
			...
			ThreadPoolManager.getInstance().schedule(new TimerToSecondStage(), 2000L); // 22.1 secs for movie
		}
	}

	private class TimerToSecondStage implements Runnable
	{
		@Override
		public void run() 
		{
	...
	private class KnightCaptainSpawnMovie implements Runnable
	{
		@Override
		public void run() 
		{
			...
			ThreadPoolManager.getInstance().schedule(new KnightCaptainSpawn(), 2000L);
		}
	}

	private class KnightCaptainSpawn implements Runnable
	{
		@Override
		public void run() 
		{
	...
	private class PreThirdStage implements Runnable
	{
		@Override
		public void run() 
		{
	...
	private class PreThirdStageM implements Runnable
	{
		@Override
		public void run() 
		{
			...
			ThreadPoolManager.getInstance().schedule(new ThirdStage(), 2000L); // 21.5 secs for movie
		}
	}
	...
	private class PreForthStage implements Runnable
	{
		@Override
		public void run() 
		{
			...
			//}
			ThreadPoolManager.getInstance().schedule(new ForthStage(), 2000L); // 27 secs for movie
		}
	}

	private class ForthStage implements Runnable
	{
		@Override
		public void run() 
		{
	...
	private class FreyaDeathStage implements Runnable
	{
		@Override
		public void run() 
		{
			...
			for(Player p : getPlayers())
			{
				QuestState qs = p.getQuestState(10286);//與希露再次見面 覺醒任務
				if(qs != null && qs.getCond() == 6)
					qs.setCond(7);
				//p.startScenePlayer(SceneMovie.FREYA_ENDING_A);
			}
			ThreadPoolManager.getInstance().schedule(new ConclusionMovie(), 2000L); // 16 secs for movie
		}
	}

	private class ConclusionMovie implements Runnable
	{
		@Override
		public void run() 
		{
			...
			ThreadPoolManager.getInstance().schedule(new InstanceConclusion(), 2000L); // 56 secs for movie
		}
	}

	private class InstanceConclusion implements Runnable
	{
		@Override
		public void run() 
		{
			startCollapseTimer(5 * 60 * 1000,false);
			doCleanup();
			for(Player p : getPlayers())

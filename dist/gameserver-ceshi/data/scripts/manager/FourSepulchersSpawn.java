...
import l2s.gameserver.model.Player;//四大陵墓寶箱
import l2s.gameserver.model.Spawner;
import l2s.gameserver.model.instances.DoorInstance;
import l2s.gameserver.model.instances.NpcInstance;
import l2s.gameserver.templates.npc.NpcTemplate;
import l2s.gameserver.utils.ItemFunctions;//四大陵墓寶箱
import l2s.gameserver.utils.NpcUtils;
import l2s.gameserver.utils.ReflectionUtils;

import org.napile.primitive.sets.IntSet;//四大陵墓寶箱
import org.napile.primitive.sets.impl.HashIntSet;//四大陵墓寶箱
...
public class FourSepulchersSpawn
{
	...
	private static void initFixedInfo()
	{
		...
		KEY_BOX_NPC.put(18120, 31455);//鑰匙箱子
		KEY_BOX_NPC.put(18121, 31455);//鑰匙箱子
		KEY_BOX_NPC.put(18122, 31455);//鑰匙箱子
		KEY_BOX_NPC.put(18123, 31455);//鑰匙箱子
		KEY_BOX_NPC.put(18124, 31456);//鑰匙箱子
		KEY_BOX_NPC.put(18125, 31456);//鑰匙箱子
		KEY_BOX_NPC.put(18126, 31456);//鑰匙箱子
		KEY_BOX_NPC.put(18127, 31456);//鑰匙箱子
		KEY_BOX_NPC.put(18128, 31457);//鑰匙箱子
		KEY_BOX_NPC.put(18129, 31457);//鑰匙箱子
		KEY_BOX_NPC.put(18130, 31457);//鑰匙箱子
		KEY_BOX_NPC.put(18131, 31457);//鑰匙箱子
		KEY_BOX_NPC.put(18145, 31458);
	...
	//四大陵墓寶箱--
	private static final IntSet _rewardedPlayers = new HashIntSet();
	
	public static boolean rewardSupplies(Player player)//這裡是給物品，讓玩家自己挑，下面是自己設置要給啥
	{
		...
		int rnd = Rnd.get(100);
		if(rnd < 50)
			ItemFunctions.addItem(player, 57, Rnd.get(99999, 9999999));
		else if(rnd < 70)
			ItemFunctions.addItem(player, 91393, Rnd.get(1, 1));
		else if(rnd < 85)
			ItemFunctions.addItem(player, 91394, Rnd.get(1, 1));
		else if(rnd < 95)
			ItemFunctions.addItem(player, 91463, Rnd.get(1, 3));
		else
			ItemFunctions.addItem(player, 91462, Rnd.get(1, 3));

...
public final class _416_PathToOrcShaman extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("QuestMonsterDurkaSpirit_Fail"))
			for(NpcInstance n : GameObjectsStorage.getNpcs(false, QuestMonsterDurkaSpirit))
				n.deleteMe();
		return htmltext;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 9)
		{
			if (npcId == VenomousSpider || npcId == ArachnidTracker)
			{
				...
				{
					if (GameObjectsStorage.getNpcs(true, QuestMonsterDurkaSpirit).isEmpty())
					{
			...
			else if (npcId == QuestMonsterDurkaSpirit)
			{
				...
				for (NpcInstance qnpc : GameObjectsStorage.getNpcs(false, QuestMonsterDurkaSpirit))
					qnpc.deleteMe();
				if (cond == 9) {

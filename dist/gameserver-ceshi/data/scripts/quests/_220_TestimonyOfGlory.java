...
public final class _220_TestimonyOfGlory extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30615-04.htm"))
		{
			...
			int spawn = 0;
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, PashikasSonOfVoltarQuestMonster);
			if(!isQuests.isEmpty())
				spawn = 1;
			isQuests = GameObjectsStorage.getNpcs(true, VultusSonOfVoltarQuestMonster);
			if(!isQuests.isEmpty())
				spawn = 1;
		...
		{
			//Проверяем есть ли в мире уже квест монстры
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, EnkuOrcOverlordQuestMonster);
			if(!isQuests.isEmpty())
			{
		...
		{
			//Проверяем есть ли в мире уже квест монстры
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, MakumBugbearThugQuestMonster);
			if(!isQuests.isEmpty())
			{
		...
		{
			//Тележка...
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, RevenantOfTantosChief);
			if(!isQuests.isEmpty())
	...
	{
		int deleted = 0;
		for(NpcInstance npc : GameObjectsStorage.getNpcs(true, mobId))
		{
			npc.deleteMe();
			deleted++;
			if(deleted >= count)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 4)
		{
			...
			else if(npcId == VultusSonOfVoltarQuestMonster)
			{
				...
				List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, VultusSonOfVoltarQuestMonster);
				if(!isQuests.isEmpty())
					isQuests.get(0).deleteMe();

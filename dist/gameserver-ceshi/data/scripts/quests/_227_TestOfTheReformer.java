...
public final class _227_TestOfTheReformer extends Quest
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(GameObjectsStorage.getNpcs(true, CrimsonWerewolf).isEmpty())
			{
				st.setCond(12, false);
		...
		{
			if(GameObjectsStorage.getNpcs(true, KrudelLizardman).isEmpty())
			{
				st.setCond(15, false);
		...
		{
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, Aruraune);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			if(st.getCond() == 2)
				st.setCond(1);
			return null;
		}
		...
		{
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, OlMahumInspector);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			isQuests = GameObjectsStorage.getNpcs(true, OlMahumPilgrimNPC);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			if(st.getCond() == 6)
				st.setCond(5);
		...
		{
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, OlMahumBetrayer);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			if(st.getCond() == 8)
				st.setCond(7);
		...
		{
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, CrimsonWerewolf);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			if(st.getCond() == 12)
				st.setCond(11);
		...
		{
			List<NpcInstance> isQuests = GameObjectsStorage.getNpcs(true, KrudelLizardman);
			if(!isQuests.isEmpty())
				isQuests.get(0).deleteMe();
			if(st.getCond() == 15)
				st.setCond(14);
		...
		switch (npcId)
		{
			...
			case Katari:
			{
				if(cond == 5 || cond == 6)
				{
					...
					st.setCond(6, false);
					List<NpcInstance> piligrims = GameObjectsStorage.getNpcs(true, OlMahumPilgrimNPC);
					List<NpcInstance> inspectors = GameObjectsStorage.getNpcs(true, OlMahumInspector);
					if(piligrims.isEmpty() && inspectors.isEmpty())
					{
						NpcInstance piligrim = addSpawn(OlMahumPilgrimNPC, npc.getLoc(), 60, 0);
						NpcInstance inspector = addSpawn(OlMahumInspector, piligrim.getLoc(), 60, 0);
				...
				else if(cond == 7 || cond == 8)
				{
					...
					if(cond == 7)
					{
						...
						List<NpcInstance> pilgrims = GameObjectsStorage.getNpcs(true, OlMahumPilgrimNPC);
						if(!pilgrims.isEmpty())
							pilgrims.get(0).deleteMe();

						st.cancelQuestTimer("Wait2");
					}

					if(GameObjectsStorage.getNpcs(true, OlMahumBetrayer).isEmpty())
					{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == NamelessRevenant && (cond == 1 || cond == 2))
		{
			...
			else if(GameObjectsStorage.getNpcs(true, Aruraune).isEmpty())
			{

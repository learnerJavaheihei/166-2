...
//第31行也需要改成 4
public class AddItemsOneTimes extends ScriptPCCouponHandler//這一個是給玩家觀注公眾號用的
{
	private static final Logger _log = LoggerFactory.getLogger(PCCouponHandler.class);
	private static final String SELECT_PCCAFE_CODE = "SELECT serial_code FROM pccafe_coupons WHERE type= 3 and login=?";
	@Override
	...
	public boolean useCoupon(Player player, String value)
	{
		...
		{
			player.sendMessage("已領取限定帳號獎勵。");//這一個是類型3出的顯示文字，加別的類型自行改改。
			return false;
		}

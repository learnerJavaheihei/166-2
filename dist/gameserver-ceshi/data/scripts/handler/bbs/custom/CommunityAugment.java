...
public class CommunityAugment extends CustomCommunityHandler {
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsaugment".equals(cmd)) {
			...
			HtmTemplates tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/augment.htm", player);
			html = tpls.get(0);
			...
			if(equippedWeapon == null)
				...
				if ("buy".equalsIgnoreCase(cmd2)) {
					...
					Map<SkillEntry, OptionDataTemplate> augmentSkills = getVariationSkills(equippedWeapon);
					if (augmentSkills.isEmpty())
						...
						SkillEntry[] skillEntries = augmentSkills.keySet().toArray(new SkillEntry[augmentSkills.size()]);
						for (int i = ((page - 1) * MAX_SKILLS_ON_PAGE); i < Math.min(skillEntries.length, (page * MAX_SKILLS_ON_PAGE)); i++) {
							SkillEntry skillEntry = skillEntries[i];
							String column = tpls.get(330);
							column = column.replace("<?skill_level?>", String.valueOf(skillEntry.getDisplayLevel()));
							String skillName = skillEntry.getName(player);
							column = column.replace("<?skill_name?>", skillName.substring(skillName.indexOf(": ") + 2));
							column = column.replace("<?skill_icon?>", skillEntry.getTemplate().getIcon());
							column = column.replace("<?variation_id?>", String.valueOf(augmentSkills.get(skillEntry).getId()));
	...
	private Map<SkillEntry, OptionDataTemplate> getVariationSkills(ItemInstance weapon) {
		Map<SkillEntry, OptionDataTemplate> skills = new HashMap<>();
		if(weapon == null)
			return skills;
		...
		for(OptionDataTemplate optionData : variations.valueCollection()) {
			for(SkillEntry skillEntry : optionData.getSkills())
				skills.put(skillEntry, optionData);

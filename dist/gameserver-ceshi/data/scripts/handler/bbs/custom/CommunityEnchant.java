...
public class CommunityEnchant extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsenchant".equals(cmd))
		{
			...
			HtmTemplates tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/enchant.htm", player);
			html = tpls.get(0);
			...
			ItemInstance item = inventory.getPaperdollItem(Inventory.PAPERDOLL_PENDANT);
			if(item != null)
			{
				...
				if(itemObjectId > 0)
				{
					...
					if(item != null && /*item.isEquipped() && */item.getOwnerId() == player.getObjectId())
					{
						...
						if(enchantCost != null)
						{
							...
							{
								player.sendMessage(player.isLangRus() ? "選擇的物品已被強化到最大。" : "选择的物品已被强化到最大。");
							}
							else
							{
								...
								if(feeItemId == 0)
								{
									player.sendMessage(player.isLangRus() ? "所選擇的物品無法強化。" : "所选择的物品无法强化。");
								}
								else
								{
									...
									if(st.hasMoreTokens())
									{
										...
										if(cmd1.equals("enchant"))
										{
											if(feeItemId <= 0 || feeItemCount <= 0 || ItemFunctions.deleteItem(player, feeItemId, feeItemCount))
											{
												...
												player.sendMessage(player.isLangRus() ? "恭喜您！道具強化成功，已成為強化「+" + enchantLevel + "」。" : "恭喜您！道具强化成功，已成为强化「+" + enchantLevel + "」。");
											}
											else
												player.sendMessage(player.isLangRus() ? "強化費用不足。" : "强化费用不足。");
										}
									}
							...
							player.sendMessage(player.isLangRus() ? "所選擇的物品無法強化。" : "所选择的品无法强化。");
					}
				}
			}

...
import handler.bbs.custom.communitybuffer.*;
...
public class CommunityBuffer extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		{
			player.sendMessage(player.isLangRus() ? "無法使用此功能" : "无法使用此功能。");
			player.sendPacket(ShowBoardPacket.CLOSE);
			return;
		}
		...
		if(bypass.startsWith("_bbscancel"))
		{
			...
			{
				if(!abnormal.isOffensive() && (BBSConfig.BUFF_SERVICE_CAN_CANCEL_NOT_DISPELLABLE_BUFFS || abnormal.getSkill().isSelfDispellable()) && !abnormal.getSkill().hasEffect(EffectUseType.NORMAL, "Transformation") && !player.isSpecialAbnormal(abnormal.getSkill()))
					abnormal.exit();
			}
		...
		if(bypass.startsWith("_cbbsbuffer"))
		{
			...
			{
				player.sendMessage(player.isLangRus() ? "奧林匹亞競賽不提供此功能。" : "奥林匹亚竞赛不提供此功能。");
				player.sendPacket(ShowBoardPacket.CLOSE);
				return;
			}
			...
			{
				player.sendMessage(player.isLangRus() ? "渾沌祭典不提供此功能。" : "浑沌祭典不提供此功能。");
				player.sendPacket(ShowBoardPacket.CLOSE);
				return;
			}*/
			...
			{
				player.sendMessage(player.isLangRus() ? "戰鬥時不提供此功能。" : "战斗时不提供此功能。");
				player.sendPacket(ShowBoardPacket.CLOSE);
				return;
			}
		...
		html = html.replace("<?fee_item?>", price > 0 ? HtmlUtils.htmlItemName(BBSConfig.BUFF_SERVICE_COST_ITEM_ID) : (player.isLangRus() ? "免費" : "免费"));
	...
	private static void doBuff(Playable target, Set<BuffSkill> buffs, long price) {
		...
		ThreadPoolManager.getInstance().execute(() ->{
			...
			for(BuffSkill nextbuff : buffs)
			{
				...
				nextbuff.getSkill().getEffects(target, target, nextbuff.getTimeAssign() * 60 * 1000, nextbuff.getTimeModifier());
	...
	private String pageGet(Player player, String[] var)
	{
		...
		if(buffallset)
		{
			...
			{
				if(player.isLangRus())
					return "<table><tr><td align=\"center\"><font color=\"FF3355\">「" + setId + "」未發現。</font></td></tr></table>";
				else
					return "<table><tr><td align=\"center\"><font color=\"FF3355\">「" + setId + "」未发现。</font></td></tr></table>";
			}
	...
	private static String pageList(Player player, String[] var)
	{
		...
		if(!_all)
		{
			...
			{
				if(player.isLangRus())
					return "<table><tr><td align=\"center\"><font color=\"FF3355\">「" + setId + "」未發現。</font></td></tr>";
				else
					return "<table><tr><td align=\"center\"><font color=\"FF3355\">「" + setId + "」未发现。</font></td></tr>";
			}
		...
		for(int i = pageIdx * pageMax; i < buffSetSkills.size(); )
		{
			...
			if(player.isLangRus())
				buff_str += "<td fixwidth=\"165\"><table width=\"160\" border=\"0\"><tr><td width=\"160\">" + makeNameBlock(player, buffSkill) + "</td></tr><tr><td width=\"160\"><font color=\"LEVEL\">等級 " + buffSkill.getSkill().getLevel() + "</font></td></tr></table></td>";
			else
				buff_str += "<td fixwidth=\"165\"><table width=\"160\" border=\"0\"><tr><td width=\"160\">" + makeNameBlock(player, buffSkill) + "</td></tr><tr><td width=\"160\"><font color=\"LEVEL\">等级 " + buffSkill.getSkill().getLevel() + "</font></td></tr></table></td>";
		...
		if(!_all && /*pageNext.isEmpty() && pagePrev.isEmpty() && */tds.size() > 0)
		{
			...
			if(player.isLangRus())
			{
				result.append(htmlButton("角色", 100, 25, "get", 0, var[1], buffSet.getId()) + "</td>");
				if(player.hasServitor())
					result.append("<td align=\"center\">" + htmlButton("召喚獸", 100, 25, "get", 2, var[1], buffSet.getId()) + "</td>");
			}
			else
			{
				result.append(htmlButton("角色", 100, 25, "get", 0, var[1], buffSet.getId()) + "</td>");
				if(player.hasServitor())
					result.append("<td align=\"center\">" + htmlButton("召唤兽", 100, 25, "get", 2, var[1], buffSet.getId()) + "</td>");
			}
		...
		{
			if(player.isLangRus())
				result.append("<table><tr><td align=\"center\">" + htmlButton("編輯", 125, 25, "editset", "edit", buffSet.getId(), 0) + "</td></tr></table>");
			else
				result.append("<table><tr><td align=\"center\">" + htmlButton("编辑", 125, 25, "editset", "edit", buffSet.getId(), 0) + "</td></tr></table>");
		}
		...
		if(!pagePrev.isEmpty() || !pageNext.isEmpty())
		{
			...
			if(player.isLangRus())
				pagesNavigation.append("第 ");
			else
				pagesNavigation.append("第 ");
			pagesNavigation.append(pageIdx + 1);
			if(player.isLangRus())
				pagesNavigation.append(" 頁");
			else
				pagesNavigation.append(" 页");
		...
		result.append("<br><table><tr><td align=\"center\">" + htmlButton((player.isLangRus() ? "返回" : "返回"), 125, 25, "0") + "</td></tr></table>");
	...
	private static String pageEdit(Player player, String[] var)
	{
		...
		if(var[1].equalsIgnoreCase("delconf"))
		{
			...
			if(player.isLangRus())
				result += "<td width=\"400\" align=\"center\"><font color=\"FF3355\">您確定要刪除「" + name + "」嗎？</font></td>";
			else
				result += "<td width=\"400\" align=\"center\"><font color=\"FF3355\">您确定要删除「" + name + "」吗？</font></td>";
		...
		if(var[1].equalsIgnoreCase("new"))
		{
			if(buffSets.size() >= BBSConfig.BUFF_SERVICE_MAX_BUFF_SETS_PER_CHAR)
				return player.isLangRus() ? "<table><tr><td><font color=\"FF3355\">達到上限數量。</font></td></tr></table>" : "<table><tr><td><center><font color=\"FF3355\">达到上限数量。</font></td></tr></table>";
			...
			{
				if(player.isLangRus())
					return "<table><tr><td><font color=\"FF3355\">您必須輸入名稱。</font></td></tr></table>";
				else
					return "<table><tr><td><font color=\"FF3355\">您必须输入名称。</font></td></tr></table>";
			}
			...
			if (buffSet == null) {
				if (player.isLangRus())
					return "<table><tr><td><font color=\"FF3355\">「" + setId + "」未發現。</font></td></tr></table>";
				else
					return "<table><tr><td><font color=\"FF3355\">「" + setId + "」未发现。</font></td></tr></table>";
			}
			...
			if (var[1].equalsIgnoreCase("rem"))
				...
				if (!availableBuffs.isEmpty()) {
					...
					result += "<table><tr>";
					if (player.isLangRus())
						result += "<td width=\"300\" align=\"center\"><font color=\"DDD3B6\">編輯「" + name + "」</font></td>";
					else
						result += "<td width=\"300\" align=\"center\"><font color=\"DDD3B6\">编辑「" + name + "」</font></td>";
					result += "</tr></table>";
					StringBuilder pagesNavigation = new StringBuilder();
					if (!pagePrev.isEmpty() || !pageNext.isEmpty()) {
						pagesNavigation.append("<table><tr>");
					...
					result += "<br><table><tr><td>" + htmlButton((player.isLangRus() ? "返回" : "返回"), 125, 25, "editset", "edit", buffSet.getId(), 0) + "</td></tr></table>";
					return result;
				}
			}
		}
		...
		if(player.isLangRus())
		{
			result += "<td width=\"300\" align=\"center\"><font color=\"DDD3B6\">編輯「 " + name + "」</font></td></tr></table>";
			result += "<table><tr>";
			if(availableBuffs.size() < BBSConfig.BUFF_SERVICE_MAX_BUFFS_IN_SET)
				result += "<td width=\"120\" align=\"center\">" + htmlButton("添加輔助", 120, 25, "editset", "add", buffSet.getId(), 0, 0) + "</td>";
			result += "<td width=\"120\" align=\"center\">" + htmlButton("刪除", 120, 25, "editset", "delconf", buffSet.getId()) + "</td>";
			result += "</tr></table>";
		}
		else
		{
			result += "<td width=\"300\" align=\"center\"><font color=\"DDD3B6\"编辑「" + name + "」</font></td></tr></table>";
			result += "<table><tr>";
			if(availableBuffs.size() < BBSConfig.BUFF_SERVICE_MAX_BUFFS_IN_SET)
				result += "<td width=\"120\" align=\"center\">" + htmlButton("添加辅助", 120, 25, "editset", "add", buffSet.getId(), 0, 0) + "</td>";
			result += "<td width=\"120\" align=\"center\">" + htmlButton("删除", 120, 25, "editset", "delconf", buffSet.getId()) + "</td>";
			result += "</tr></table>";
		}
		...
		result += "<br><table><tr><td>" + htmlButton((player.isLangRus() ? "返回" : "返回"), 125, 25, "list", "2_0", buffSet.getId()) + "</td></tr></table>";
	...
	private static String pageMain(Player player)
	{
		...
		if(player.isLangRus())
			result.append(htmlButton("全部輔助", 200, 30, "list", "0_0"));
		else
			result.append(htmlButton("全部辅助", 200, 30, "list", "0_0"));
		...
		result.append("<td align=\"center\">");
		if(player.isLangRus())
			result.append("<br><br><font color=\"DDD3B6\">個人設定</font><br>");
		else
			result.append("<br><br><font color=\"DDD3B6\">个人设定</font><br>");
		result.append("<table>");
		...
		if(player.isLangRus())
			result.append(htmlButton("保存", 100, 25, "editset", "new", "$newfname"));
		else
			result.append(htmlButton("保存", 100, 25, "editset", "new", "$newfname"));

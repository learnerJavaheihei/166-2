...
import l2s.gameserver.model.entity.auction.AccessoryItemType;
import l2s.gameserver.model.entity.auction.ArmorItemType;
import l2s.gameserver.model.entity.auction.Auction;
import l2s.gameserver.model.entity.auction.AuctionItemTypes;
import l2s.gameserver.model.entity.auction.AuctionManager;
import l2s.gameserver.model.entity.auction.EtcAuctionItemType;
import l2s.gameserver.model.entity.auction.PetItemType;
import l2s.gameserver.model.entity.auction.SuppliesItemType;
import l2s.gameserver.model.entity.auction.WeaponItemType;
...
public class CommunityAuction extends CustomCommunityHandler
{
    private static final Logger _log = LoggerFactory.getLogger(CommunityAuction.class);
    ...
    protected void doBypassCommand(Player player, String bypass)
    {
        ...
        if(!Config.ENABLE_AUCTION_SYSTEM)
        {
            ...
            ShowBoardPacket.separateAndSend(msg, player);
        }

        if("maillist".equals(cmd))
        {
            html = HtmCache.getInstance().getHtml("scripts/handler/bbs/pages/auction/mail_list.htm", player);
                    ...
                    if(action == 1)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("确定要买吗 " + quantity + ' ' + item.getName(player) + " for " + Util.formatAdena(realPrice) + " adena?");
                                player.ask(packet, new ButtonClick(player, item, Buttons.Buy_Item, quantity));
                            }
                        }
                    }
                    // sending buy_item html
                    html = HtmCache.getInstance().getHtml("scripts/handler/bbs/pages/auction/mail_buy_item.htm", player);
        ...
        else if(cmd.equals("bbsNewAuction"))
        {
            ...
            {
                player.sendMessage("在私人商店经营时，您不能打开新的拍卖。");
                return;
            }
            ...
            if(buttonClicked != null)
            {
                ...
                if(buttonClicked.equals("0"))
                {
                    ...
                    if(error)
                    {
                        player.sendMessage("填写所有信息！");
                    }
                    else if(item == null)
                    {
                        player.sendMessage("道具不再存在！");
                    }
                    else
                    {
                        ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("你确定要卖吗 " + item.getName(player) + "?");
                ...
                else if(buttonClicked.equals("1"))
                {
                    ...
                    {
                        ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("是否确定要取消 " + item.getName(player) + " 拍卖?");
            ...
            html = HtmCache.getInstance().getHtml("scripts/handler/bbs/pages/auction/mail_new_auction.htm", player);
    ...
    private String fillAuctionListPage(Player player, String html, int page, int[] itemTypes, String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort)
    {
        ...
        List<Auction> auctions = getRightAuctions(player, allAuctions, itemTypes, itemGrade, search);
        ...
        for(int i = 10 * (page - 1); i < Math.min(auctions.size(), 10 * (page)); i++)
        {
            ...
            builder.append("<table border=0 cellspacing=1 cellpadding=0 width=530 height=30 bgcolor=").append((i % 2) == 1 ? "1a1914" : "23221d").append(">");

            builder.append("<tr><td width=280 height=25><table width=280 height=30><tr>");

            builder.append("<td>" + makeIconBlock(item.getTemplate().getIcon(), "_bbsAuction_ %page% _ %type% _ %grade% _ %search% _ %itemSort% _ %gradeSort% _ %quantitySort% _ %priceSort% _ 0 _ " + auction.getAuctionId()) + "</td>");
            builder.append(getItemName(player, item, 248, 25));
            builder.append("</tr></table></td><td width=75 height=30><center>");

            if(item.getGrade() != ItemGrade.NONE)
                builder.append("<img src=").append(getGradeIcon(item.getGrade())).append(" width=15 height=15>");
            else
                builder.append("None");

            builder.append("</center></td><td width=75 height=30>");
            builder.append("<center>").append(auction.getCountToSell()).append("</center>");
            builder.append("</td><td width=100 height=30 align=right>");
            builder.append(Util.formatAdena(auction.getPricePerItem()));
        ...
        html = html.replace("%adena%", Util.formatAdena(player.getAdena()));
        return html;
    }
    ...
    private String fillPurchasePage(Player player, String html, int page, int[] itemTypes, String itemGrade, String search, int itemSort, int gradeSort, int quantitySort, int priceSort, int auctionId)
    {
        ...
        // Name
        builder.append("<center>" + choosenItem.getName(player));
        ...
        // Grade
        builder.append("<br><center><img src=").append(getGradeIcon(choosenItem.getGrade())).append(" width=15 height=15>");
        ...
        html = html.replace("%icon%", "<img src=" + choosenItem.getTemplate().getIcon() + " width=32 height=32>");
        html = html.replace("%fullName%", "<table width=240 height=50><tr>" + getItemName(player, choosenItem, 240, 50, (auction.getCountToSell() > 1 ? " x" + auction.getCountToSell() : "")) + "</tr></table>");
        html = html.replace("%quantity%", (auction.getCountToSell() > 1 ? "<edit var=\"quantity\" value=\" \" width=160 height=12>" : "<center><font color=94775b>1</font></center>"));

        if(auction.getCountToSell() <= 1)
            html = html.replace("$quantity", "1");

        html = html.replace("%pricePerItem%", "<font color=FFFFFF>" + String.valueOf(Util.formatAdena(auction.getPricePerItem())) + "</font>");
        html = html.replace("%totalPrice%", "<font color=FFFFFF>" + String.valueOf(Util.formatAdena(auction.getCountToSell() * auction.getPricePerItem())) + "</font>");
        html = html.replace("%totalAdena%", "<font color=FFFFFF>" + String.valueOf(Util.formatAdena(player.getAdena())) + "</font>");
    ...
    private String fillNewAuctionPage(Player player, String html, boolean newItem, int currentItem, int line)
    {
        ...
        for(int i = 6 * (line); i < (6 * (line + 3)); i++)
        {
            ...
            if(item != null)
                builder.append(makeIconBlock(item.getTemplate().getIcon(), "_bbsNewAuction_ n" + item.getObjectId() + " _ " + line));
            else
                builder.append("<br>");
...
//            builder.append("</tr>");
        ...
        for(; i < 10; i++)
        {
            ...
            // starting line
            builder.append("<tr><td width=260><table width=260><tr><td width=32>");
            // button with image of item icon
            builder.append(makeIconBlock(item.getTemplate().getIcon(),"_bbsNewAuction_ c" + item.getObjectId() + " _ " + line + " _ 1"));
            builder.append("</td>");
            // getting item name
            builder.append(getItemName(player,item, 228, 25));
        ...
        html = html.replace("%totalPrice%", Util.formatAdena(0));
        html = html.replace("%saleFee%", Util.formatAdena(Config.AUCTION_FEE));
        html = html.replace("%adena%", Util.formatAdena(player.getAdena()));
        ...
        html = html.replace("%choosenImage%", (choosenItem != null ? "<img src=" + choosenItem.getTemplate().getIcon() + " width=32 height=32>" : ""));
        html = html.replace("%choosenItem%", (choosenItem != null ? (getItemName(player, choosenItem, 180, 45, (choosenItem.getCount() > 1 ? " x" + choosenItem.getCount() : ""))) : ""));
        html = html.replace("%quantity%", ((choosenItem == null) || (choosenItem.getCount() > 1) ? "<edit var=\"quantity\" value=\" \" width=160 height=12>" : "<center>1</center>"));
        html = html.replace("%NewAuctionButton%", (choosenItem != null ? "<center><button value=\"上架\" action=\"bypass _bbsNewAuction_ " + (newItem ? "n" : "c") + currentItem + " _ " + line + " _ 0 _ " + ((choosenItem.getCount() > 1) ? "$quantity" : "1") + " _ $sale_price\" width=90 height=30 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></center>" : ""));
     ...
     * @return
     */
    private List<Auction> getRightAuctions(Player player, Collection<Auction> allAuctions, int[] splitedTypes, String itemGrade, String search)
    {
        ...
        for(Auction auction : allAuctions)
        {
            ...
            {
                if(!auction.getItem().getGrade().name().equalsIgnoreCase(itemGrade))
                    continue;
            }
            if(search != null)
            {
                if(!StringUtils.containsIgnoreCase(auction.getItem().getName(player), search))
                    continue;
            }
    ...
    private List<ItemInstance> getItemsToAuction(Player player)
    {
        ...
        {
            if(item.getTemplate().isAdena())
                continue;

            if(!item.getTemplate().isTradeable())
                continue;

            if(item.getLocation() == ItemLocation.AUCTION)
                continue;
     ...
     * @return
     */
    private String getItemName(Player player, ItemInstance item, int windowWidth, int windowHeight, String... addToItemName)
    {
        StringBuilder builder = new StringBuilder();
        ...
        String itemName = item.getName(player);
    ...
    private static String makeIconBlock(String icon, String link) {
        StringBuilder sb = new StringBuilder();
        if(link != null) {
            sb.append("<table background=\"" + icon + "\" width=34 height=34 border=0 cellpadding=0 cellspacing=0>");
            sb.append("<tr><td align=center><button value=\" \" action=\"bypass " + link + "\" width=34 height=34 back=\"L2UI_CH3.inventory_outline_over\" fore=\"L2UI_CH3.inventory_outline\"></td></tr>");
            sb.append("</table>");
        }
        else {
            sb.append("<table background=\"" + icon + "\" width=32 height=32 border=0 cellpadding=0 cellspacing=0>");
     ...
     * @return
     */
    private String getGradeIcon(ItemGrade grade)
    {
        ...
        public void sayYes()
        {
            switch (_button)
            {
                case New_Auction:
                    ...
                    {
                        doBypassCommand(_player, "_bbsNewAuction_ c0 _ 0");
                        return;
                    }
                ...
                case Buy_Item:
                    ...
                    doBypassCommand(_player, "_maillist");
                    break;
            }
        }
    ...
    private static class ItemNameComparator implements Comparator<Auction>, Serializable
    {
        ...
        public int compare(Auction o1, Auction o2)
        {
            ...
            if(player != null) {
                if (_rightOrder)
                    return o1.getItem().getName(player).compareTo(o2.getItem().getName(player));
                return o2.getItem().getName(player).compareTo(o1.getItem().getName(player));
            }
            else {
                if (_rightOrder)
    ...
    private static class GradeComparator implements Comparator<Auction>, Serializable
    {
        ...
        public int compare(Auction o1, Auction o2)
        {
            int grade1 = o1.getItem().getGrade().ordinal();
            int grade2 = o2.getItem().getGrade().ordinal();
            if(_rightOrder)

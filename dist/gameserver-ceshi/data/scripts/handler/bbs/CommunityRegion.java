	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("bbsloc".equals(cmd))
		{
			...
			for(int townId = 0; townId < _towns.length; townId++)
			{
				...
				for(Player seller : GameObjectsStorage.getPlayers(false, true))
				{
		...
		else if("bbsregview".equals(cmd))
		{
				...
				for(TradeItem ti : sl)
				{
					...
					if(item != null)
					{
						...
						if(item.getGrade() != ItemGrade.NONE)
						{
							...
							desc += item.getCrystalCount() > 0 ? (player.isLangRus() ? " 結晶: " : " 结晶: ") + item.getCrystalCount() + ";&nbsp;" : ";&nbsp;";
						}
						...
						if(item.isStackable())
							desc += player.isLangRus() ? "疊堆;&nbsp;" : "叠堆;&nbsp;";
						if(item.isShadowItem())
							desc += player.isLangRus() ? "幻象物品;&nbsp;" : "幻象物品;&nbsp;";
						if(item.isTemporal())
							desc += player.isLangRus() ? "限時物品;&nbsp;" : "限时物品;&nbsp;";
			...
			else
			{
				...
				for(ManufactureItem mi : cl)
				{
					...
					String desc = "";
					if(item.getGrade() != ItemGrade.NONE)
						desc = _grade[item.getGrade().ordinal() - 1] + (item.getCrystalCount() > 0 ? (player.isLangRus() ? " 結晶: " : " 结晶: ") + item.getCrystalCount() + ";&nbsp;" : ";&nbsp;");

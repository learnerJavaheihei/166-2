 ...
 * @author Bonux
**/
public class teshushoulie7 extends ProgressDailyMissionHandler
{
	private class HandlerListeners implements OnKillListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
				...
				for(Player p : player.getParty())
				{
					if(p.getDistance(player) <= 1500)
					{
						if(player != null && victim.isMonster())
						{
							if(p == playerLeader)
							{
								if(MapUtils.regionX(victim) == 25 && MapUtils.regionY(victim) == 19)
			...
			else
			{
				if(player != null && victim.isMonster())
				{
					if(MapUtils.regionX(victim) == 25 && MapUtils.regionY(victim) == 19) // Забытый Остров

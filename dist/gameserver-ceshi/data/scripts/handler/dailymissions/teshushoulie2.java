 ...
 * @author Bonux
**/
public class teshushoulie2 extends ProgressDailyMissionHandler
{
	private class HandlerListeners implements OnKillListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
				...
				for(Player p : player.getParty())
				{
					if(p.getDistance(player) <= 1500)
					{
						if(player != null && victim.isMonster())
						{
							if(p == playerLeader)
							{
								if(MapUtils.regionX(victim) == 19 && MapUtils.regionY(victim) == 23)
			...
			else
			{
				if(player != null && victim.isMonster())
				{
					if(MapUtils.regionX(victim) == 19 && MapUtils.regionY(victim) == 23) // Забытый Остров

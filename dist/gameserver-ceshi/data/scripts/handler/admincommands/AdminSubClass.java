				...
				if(wordList.length > 1)
				{
					...
					else if(wordList[1].equalsIgnoreCase("changelist"))
					{
						...
						else
						{
							...
							for(SubClass sub : targetPlayer.getSubClassList().values())
							{
								...
								button = button.replace("<?apply_button?>", sub.isActive() ? "" : tpls.get(7)); // Активный класс нельзя применить
								button = button.replace("<?change_button?>", sub.isBase() ? "" : tpls.get(8)); // Основной класс нельзя изменить
								button = button.replace("<?delete_button?>", (sub.isBase() || sub.isActive()) ? "" : tpls.get(9));	// Основной или активный класс нельзя удалить
								button = button.replace("<?class_name?>", ClassId.VALUES[sub.getClassId()].getName(player));
					...
					else if(wordList[1].equalsIgnoreCase("addclass"))
					{
						try
						{
							...
							{
								Functions.sendDebugMessage(player, "Could not add a new '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
								return false;
							}
							Functions.sendDebugMessage(player, "Success added a new '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
							return true;
						}
						catch(Exception e)
						{
					...
					else if(wordList[1].equalsIgnoreCase("applyclass"))
					{
						try
						{
							...
							{
								Functions.sendDebugMessage(player, "Could not apply a '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
								return false;
							}
							Functions.sendDebugMessage(player, "Success applied a '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
							return true;
						}
						catch(Exception e)
						{
					...
					else if(wordList[1].equalsIgnoreCase("changeclass"))
					{
						try
						{
							...
							{
								Functions.sendDebugMessage(player, "Could not change a '" + ClassId.VALUES[oldId].getName(player) + "' to '" + ClassId.VALUES[newId].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'! The base class can not be changed.");
								return false;
							}
							Functions.sendDebugMessage(player, "Success changed a '" + ClassId.VALUES[oldId].getName(player) + "' to '" + ClassId.VALUES[newId].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
							return true;
						}
						catch(Exception e)
						{
					...
					else if(wordList[1].equalsIgnoreCase("deleteclass"))
					{
						try
						{
							...
							{
								Functions.sendDebugMessage(player, "Could not delete a '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
								return false;
							}
							Functions.sendDebugMessage(player, "Success deleted a '" + ClassId.VALUES[id].getName(player) + "' subclass from player '" + targetPlayer.getName() + "'!");
							return true;
						}
						catch(Exception e)
						{
	...
	private static String generateAvailableSubClassList(Player player, Player target, Race race, HtmTemplates tpls, int classToChange) throws Exception
	{
		...
		if(race == null)
		{
			for(Race r : Race.VALUES)
			{
				...
				{
					if(classToChange >= 0 && c.equalsOrChildOf(ClassId.VALUES[classToChange]))
						continue;
					...
					{
						if(ClassId.VALUES[subClass.getClassId()].equalsOrChildOf(c))
							continue loop;
					}
		...
		else
		{
			...
			listTable = listTable.replace("<?change_class_name?>", ClassId.VALUES[classToChange].getName(player));
		}

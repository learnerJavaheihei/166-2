	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player player)
	{
		...
		switch(c)
		{
			case admin_setskill:
				...
				try
				{
					Skill skill = SkillHolder.getInstance().getSkillByIndex(Integer.parseInt(wordList[1]), Integer.parseInt(wordList[2]));
					if(skill == null)
						return false;

					target.getPlayer().addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, skill), true);
					target.getPlayer().sendSkillList();
					target.getPlayer().sendPacket(new SystemMessagePacket(SystemMsg.YOU_HAVE_EARNED_S1_SKILL).addSkillName(skill));
				}

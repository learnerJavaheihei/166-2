...
public class AdminFence extends ScriptAdminCommand
{
		...
		switch(cmd)
		{
			case admin_addfence:
			{
				try
				{
					...
					{
						player.sendMessage("寬度和長度值必須等於或大於100。");
						sendHtml(player, 0);
						return false;
					}
					if(height < 1 || height > 3)
					{
						player.sendMessage("高度範圍值1-3。");
						sendHtml(player, 0);
						return false;
					}
					FenceInstance fence = FenceUtils.spawnFence("圍欄由GM: \"" + player.getName() + "\"", player.getX(), player.getY(), player.getZ(), width, length, height, FenceState.CLOSED, player.getReflection());
					if(fence != null)
					{
						// Сохраняем заборы только в реальном мире.
						FencesDAO.getInstance().insert(fence);
						player.sendMessage("圍欄成功添加");
					}
					else
	...
	{
		final HtmTemplates tpls = HtmCache.getInstance().getTemplates("admin/fences.htm", player);

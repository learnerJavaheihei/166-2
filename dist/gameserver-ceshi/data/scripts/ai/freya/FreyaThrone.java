...
public class FreyaThrone extends Fighter
{
	...
	protected void thinkAttack()
	{
		...
		// Ice Ball Cast
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _iceballReuseTimer < System.currentTimeMillis() && Rnd.chance(_iceballChance))
        {
		...
		if(!actor.isCastingNow() && _summonReuseTimer < System.currentTimeMillis() && Rnd.chance(_summonChance))
		{
			...
			for(NpcInstance guard : getActor().getAroundNpc(800, 100))
				//guard.altOnMagicUse(guard, SkillHolder.getInstance().getSkill(Skill_SummonElemental, 1));
		...
		// Death Sentence
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _deathsentenceReuseTimer < System.currentTimeMillis() && Rnd.chance(_deathsentenceChance))
		{
		...
		// Freya Anger
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _angerReuseTimer < System.currentTimeMillis() && Rnd.chance(_angerChance))
		{

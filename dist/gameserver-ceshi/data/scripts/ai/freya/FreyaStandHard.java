...
{
	private static final int Skill_EternalBlizzard = 6275; // 終極雪暴
	private long _eternalblizzardReuseTimer = 0; // 終極雪暴重新使用計時器
	private int _eternalblizzardReuseDelay = 50; // 終極雪暴重新使用時間

	private static final int Skill_IceBall = 6278; // 冰彈
	private long _iceballReuseTimer = 0;
	private int _iceballReuseDelay = 7;

	private static final int Skill_SummonElemental = 6277; // 精靈召喚
	private long _summonReuseTimer = 0;
	private int _summonReuseDelay = 40;

	private static final int Skill_SelfNova = 6279; // 範圍攻擊
	private long _selfnovaReuseTimer = 0;
	private int _selfnovaReuseDelay = 40;

	private static final int Skill_DeathSentence = 6280; // 死亡宣告
	private long _deathsentenceReuseTimer = 0;
	private int _deathsentenceReuseDelay = 40;

	private static final int Skill_ReflectMagic = 6282; // 魔法反射
	private long _reflectReuseTimer = 0;
	private int _reflectReuseDelay = 30;

	private static final int Skill_IceStorm = 6283; // 暴風雪
	private long _icestormReuseTimer = 0;
	private int _icestormReuseDelay = 40;

	private static final int Skill_Anger = 6285; // 冰之憤怒
	...
	protected void thinkAttack()
	{
        ...
        Creature topDamager = actor.getAggroList().getTopDamager(null);
        Creature randomHated = actor.getAggroList().getRandomHated(getMaxHateRange());
        Creature mostHated = actor.getAggroList().getMostHated(getMaxHateRange());
		//Eternal Blizzard Cast
		if(!actor.isCastingNow() && _eternalblizzardReuseTimer < System.currentTimeMillis())
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_EternalBlizzard, 1), actor, true);
		...
		// Ice Ball Cast
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _iceballReuseTimer < System.currentTimeMillis())
		{
			if(topDamager != null && !topDamager.isDead() && topDamager.isInRangeZ(actor, 1000))
			{
				actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_IceBall, 1), topDamager, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_SummonElemental, 1), actor, true);
			for(NpcInstance guard : getActor().getAroundNpc(800, 100))
				guard.altOnMagicUse(guard, SkillHolder.getInstance().getSkillEntry(Skill_SummonElemental, 1));
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_SelfNova, 1), actor, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_ReflectMagic, 1), actor, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_IceStorm, 1), actor, true);
			_icestormReuseTimer = System.currentTimeMillis() + _icestormReuseDelay * 1000L;
		}

		// Death Sentence
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _deathsentenceReuseTimer < System.currentTimeMillis())
		{
			if(randomHated != null && !randomHated.isDead() && randomHated.isInRangeZ(actor, 1000))
			{
				actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_DeathSentence, 1), randomHated, true);
				_deathsentenceReuseTimer = System.currentTimeMillis() + _deathsentenceReuseDelay * 1000L;
			}
		}

		// Freya Anger
		if(!actor.isCastingNow() && !getActor().getMovement().isMoving() && _angerReuseTimer < System.currentTimeMillis())
		{
			actor.doCast(SkillHolder.getInstance().getSkillEntry(Skill_Anger, 1), actor, true);
		...
		if(System.currentTimeMillis() - _lastFactionNotifyTime > _minFactionNotifyInterval)
		{
			...
			for(NpcInstance npc : actor.getReflection().getNpcs())
				if(npc.isMonster() && npc != actor)
					npc.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, actor.getAggroList().getMostHated(getMaxHateRange()), 5);
		}
		super.thinkAttack();
	}

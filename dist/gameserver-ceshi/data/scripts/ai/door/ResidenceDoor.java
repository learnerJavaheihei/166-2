...
public class ResidenceDoor extends DoorAI
{
	...
	public void onEvtTwiceClick(final Player player)
	{
		...
		//原始if(player.getPlayerAccess().Door || residence.getOwner() != null && player.getClan() != null && player.getClan() == residence.getOwner() && (player.getClanPrivileges() & Clan.CP_CS_ENTRY_EXIT) == Clan.CP_CS_ENTRY_EXIT)
		//調整顯示錯誤門ID--
		if(residence == null)
		{
			_log.error("residence == null : ID:" + door.getTemplate().getAIParams().getInteger("residence_id") );
		}
		if(player.getPlayerAccess().Door || (residence.getOwner() != null) && (player.getClan() != null) && (player.getClan() == residence.getOwner()) && ( (player.getClanPrivileges() & Clan.CP_CS_ENTRY_EXIT) == Clan.CP_CS_ENTRY_EXIT))
		//--調整顯示錯誤門ID
		{

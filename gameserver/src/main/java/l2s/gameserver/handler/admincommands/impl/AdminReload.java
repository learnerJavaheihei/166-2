...
public class AdminReload implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, final Player activeChar)
	{
		...
		activeChar.ask(new ConfirmDlgPacket(SystemMsg.S1, 0).addString(activeChar.isLangRus() ? "重新引導伺服器組件可能導致不可預計的後果！繼續？" : "重新引导伺服器组件可能导致不可预计的后果！继续？"), new OnAnswerListener()
		{
			@Override
			public void sayYes()
			{
		...
		switch(command)
		{
			...
			case admin_reload_locale:
			{
				...
				NpcStringHolder.getInstance().reload();

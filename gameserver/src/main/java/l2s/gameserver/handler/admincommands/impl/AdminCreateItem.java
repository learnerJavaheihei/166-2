...
import l2s.gameserver.model.GameObjectsStorage;//創造物品修改
...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	{
		admin_itemcreate,
		admin_create_item_me,//創造物品修改
		admin_create_item_target,//創造物品修改
		admin_create_item_server,//創造物品修改
		admin_hidden_item,
		admin_ci,
		admin_spreaditem,
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			case admin_itemcreate:
				...
				break;
			case admin_ci:
			case admin_create_item_me://創造物品修改--
				try
				{
					...
					{
						activeChar.sendMessage("給自己物品「" + item.getName() + "」數量「" + item_count + "」個");
					}
				}
				...
				activeChar.sendPacket(new HtmlMessage(5).setFile("admin/itemcreation.htm"));
				break;
			case admin_create_item_target://--創造物品修改
				try
				{
					if(wordList.length < 2)
					{
					...
					{
						activeChar.sendMessage("已給玩家「" + activeChar.getTarget().getName() + "」 物品「" + item.getName() + "」 數量「" + item_count + "」");
						activeChar.getTarget().getPlayer().sendMessage("管理員給您物品「" + item.getName() + "」 數量「" + item_count + "」");
					}
				}
				catch (NumberFormatException nfe)
				{
				...
				activeChar.sendPacket(new HtmlMessage(5).setFile("admin/itemcreation.htm"));
				break;
			case admin_create_item_server://給整服
				try
				{
					if (wordList.length < 2)
					{
					...
					for(Player player : GameObjectsStorage.getPlayers(false, false))
					{
						...
						player.sendMessage("管理員給您物品「" + createditem.getName() + "」 數量「" + item_count + "」");
						counts++;
					}

					activeChar.sendMessage("「" + counts + "」個玩家已取得物品。");
				}
				catch(NumberFormatException nfe)
				{
					activeChar.sendMessage("USAGE: create_item id [count]");

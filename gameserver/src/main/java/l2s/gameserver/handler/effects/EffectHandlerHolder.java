...
public class EffectHandlerHolder extends AbstractHolder
{
	...
	private EffectHandlerHolder()
	{
		...
		registerHandler(EffectDamageBlockCount.class);//修復傷害護盾效果
		...
		registerHandler(EffectMuteChance.class);//修復技能
		...
		registerHandler(EffectDispelOnAttacked.class);//20190305 增加skill 51298 受到傷害消除輔助
		...
		// Instant Effects
		registerHandler(i_add_hate.class);//攻擊欲望
		registerHandler(i_align_direction.class);//露出背後
		registerHandler(i_call_random_skill.class);//隨機技能？
		registerHandler(i_call_skill.class);//召喚某個技能
		registerHandler(i_dispel_all.class);//魔法消除全部
		...
		registerHandler(i_death_link.class);//HP越低時威力越大
		...
		registerHandler(i_get_exp.class);//可獲得EXP
		...
		registerHandler(i_p_hit.class);//在一般物理攻擊力多X倍傷害
		registerHandler(i_pledge_reputation.class);//血盟聲望
		registerHandler(i_randomize_hate.class);//使敵人陷入混亂改變攻擊目標
		registerHandler(i_refresh_instance.class);//重置即時地區
		registerHandler(i_reset_skill_reuse.class);//重置技能
		registerHandler(i_set_skill.class);//學習技能
		registerHandler(i_sp.class);//可獲得SP
		registerHandler(i_soul_shot.class);//靈魂彈
		registerHandler(i_spirit_shot.class);//魔靈彈
		registerHandler(i_spoil.class);//回收
		registerHandler(i_summon_agathion.class);//壺精召喚
		registerHandler(i_summon_cubic.class);//召喚晶體
		registerHandler(i_summon_soul_shot.class);//野獸靈魂彈
		registerHandler(i_summon_spirit_shot.class);//野獸魔靈彈
		registerHandler(i_target_cancel.class);//解除目標
		registerHandler(i_target_me.class);
		registerHandler(i_unsummon_agathion.class);//壺精封印

		// Permanent Effects
		registerHandler(p_attack_trait.class);
		registerHandler(p_block_buff_slot.class);//阻攔buff 可定義類型
		registerHandler(p_block_chat.class);
		registerHandler(p_block_debuff.class);//詛咒魔法攻擊完整的抗性
		registerHandler(p_block_escape.class);//血盟回歸限制
		registerHandler(p_block_move.class);
		registerHandler(p_block_party.class);
		registerHandler(p_block_target.class);
		registerHandler(p_critical_damage.class);//物理致命攻擊傷害
		registerHandler(p_defence_trait.class);
		registerHandler(p_get_item_by_exp.class);//贖罪時間
		registerHandler(p_heal_effect.class);//恢復HP
		registerHandler(p_magic_critical_dmg.class);//魔法致命攻擊傷害
		registerHandler(p_max_cp.class);
		registerHandler(p_max_hp.class);
		registerHandler(p_max_mp.class);
		registerHandler(p_passive.class);//使怪物的行動變為被動
		registerHandler(p_preserve_abnormal.class);//死亡時，目標的輔助魔法/詛咒魔法將不會消失，而持續維持。但，貴族的祝福與幸運符咒此兩種狀態會消失
		registerHandler(p_raid_berserk.class);
		registerHandler(p_skill_critical_damage.class);//物理技能致命傷害
		registerHandler(p_target_me.class);
		registerHandler(p_violet_boy.class);//申報狀態

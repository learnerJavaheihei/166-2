package l2s.gameserver.model.items;//漢化
import java.sql.Connection;//鑄靈系統
import java.sql.PreparedStatement;//鑄靈系統
import java.sql.ResultSet;//鑄靈系統
...
import l2s.commons.dbutils.DbUtils;//鑄靈系統
...
import l2s.gameserver.database.DatabaseFactory;//鑄靈系統
...
public final class ItemInstance extends GameObject implements JdbcEntity
{
	...
	private boolean GradeCheck = false;//鑄靈系統
	public void setEnchantLevel(int value)
	{
		...
		if(!GradeCheck)//鑄靈系統--
		{
			_myGrade = getGradeItem(getObjectId());
			GradeCheck = true;
			if(_myGrade != 0  )
			{
				if(_enchantOptions[0] != 0)
				{
					int[] enchantOptions = new int[_enchantOptions.length +1];
					enchantOptions[0] = _myGrade;
					for(int i = 1; i < _enchantOptions.length; i++)
					{
						enchantOptions[i] = _enchantOptions[i-1];
					}
	...
	private int _myGrade = 0;
	public int getMyGrade()
	{
		return _myGrade;
	}
	private void reFlashGrade()
	{
		if(_enchantOptions[0] != 0)
		{
			if(_enchantOptions.length == 1 && ((_enchantOptions[0] + 1) == _myGrade))
			{
				_enchantOptions = new int[] {_myGrade,0,0};//剛好是下一級的情況 直接改新的就行
			}
			else
			{
				boolean chek = true;
				for(int i = 0; i < _enchantOptions.length; i++)
				{
					if(_enchantOptions[i] + 1 == _myGrade)//剛好是升級情況，且裡面技能直接更新即可
					{
						_enchantOptions[i] = _myGrade;
						chek = false;
					}
				}
				if(chek)
				{
					int[] enchantOptions = new int[3];
					enchantOptions[0] = _myGrade;
					for(int i = 1; i < 3; i++)
					{
						enchantOptions[i] = _enchantOptions[i - 1];
		...
		{
			_enchantOptions = new int[] {_myGrade,0,0};
		}
	}
	public void setGrade(int value)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("REPLACE INTO items_myensoul(object_id, ensoul_id ) VALUES(?, ?)");
	...
	{
		if(getVisualId() > 0 || isTemporalItem() || isFlagLifeTime())
		...
		if(lastAttacker != null) // lastAttacker в данном случае top damager
		{
			...
			if(fromNpc != null && fromNpc.isBoss()) // На эпиках, дроп поднимает лидер CC.
			{
				...
				if(group instanceof CommandChannel)
					group = ((CommandChannel) group).getGroupLeader().getParty();
			}
	...
	{
		if(_myGrade > 0 && _enchantOptions[0] == 0)//鑄靈系統--
		{
			if(isWeapon() || isArmor())
			{
				GradeCheck = true;
	...
	public SkillEntry addEquippedSkill(Object owner, SkillEntry skill)
	{
		if(_equippedSkills == null)
			_equippedSkills = new HashMap<Object, IntObjectMap<SkillEntry>>();

		IntObjectMap<SkillEntry> skillsMap = _equippedSkills.get(owner);
		if(skillsMap == null)
		{
			skillsMap = new HashIntObjectMap<SkillEntry>();
			_equippedSkills.put(owner, skillsMap);
		}
	...
	public OptionDataTemplate addEquippedOptionData(Object owner, OptionDataTemplate optionData)
	{
		if(_equippedOptionDatas == null)
			_equippedOptionDatas = new HashMap<Object, IntObjectMap<OptionDataTemplate>>();

		IntObjectMap<OptionDataTemplate> optionDataMap = _equippedOptionDatas.get(owner);
		if(optionDataMap == null)
		{
			optionDataMap = new HashIntObjectMap<OptionDataTemplate>();
			_equippedOptionDatas.put(owner, optionDataMap);
		}

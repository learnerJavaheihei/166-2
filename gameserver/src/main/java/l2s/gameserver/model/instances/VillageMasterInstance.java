...
import l2s.gameserver.network.l2.s2c.ExShowUpgradeSystem;//升級系統
import l2s.gameserver.network.l2.s2c.ExShowUpgradeSystemNormal;//升級系統
...
public final class VillageMasterInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(command.equals("disband_clan_check"))
		{
			if(!player.isClanLeader())
			{
				showChatWindow(player, "pledge/pl008.htm", false);//修復 顯示對話頁面 原始頁面 pl_err_master
				return;
			}
	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		...
		else if (ask == -2418)//升級系統--
	    {
	      if (reply == 1L) {
	        ...
	        showChatWindow(player, "villagemaster/upgrade_system001.htm", false, new Object[0]);
	      }
	    }//--升級系統
		else
			super.onMenuSelect(player, ask, reply, state);
		...
		switch(_type)
		{
			...
			case ORC_2ND_CLASS:
			{
				...
				if(classId == ClassId.WARCRYER || classId == ClassId.DOOMCRYER )
					return true;
				//修復黑暗精靈劍舞副職業切回君主職業--
				if(classId == ClassId.OVERLORD  || classId == ClassId.DOMINATOR)//20180905修正
					return true;
				//--修復黑暗精靈劍舞副職業切回君主職業
				return false;
			}

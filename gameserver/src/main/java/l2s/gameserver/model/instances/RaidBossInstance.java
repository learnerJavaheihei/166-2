...
public class RaidBossInstance extends MonsterInstance
{
	...
	protected void onDeath(Creature killer)
	{
		...
		if(killer != null && killer.isPlayable())
		{
			...
			//狩獵BOSS獲得狩獵禮盒--
			int MaxLevel = Math.max(getLevel() + 6, 86);//設置的最大等級 就是首領等級 +8 且不超過85級
			int MinLevel = Math.min(getLevel() - 8, 71);//設置的最小等級 //首領等級 - 16 且不小於1級
			int itembos = 0;
			if(getLevel() >= 80 && getLevel() <81)
			{
			itembos = 31523;
			}
			if(getLevel() >= 82 && getLevel() <83)
			{
			itembos = 31522;
			}
			if (player.isInParty())//如果玩家是組隊狀態
			{
				if(player.isInParty() && player.getParty().getCommandChannel() != null)//如果玩家是聯軍狀態
				{
					//這裡是聯軍階段
					Player playerLeader = player.getPlayerGroup().getGroupLeader();
					for(Player p : player.getPlayerGroup())
					{
						if((p.getLevel() < MaxLevel) && (p.getLevel() > MinLevel))//判斷所有的玩家比較玩家跟BOSS的等級差再給物品。
						{
							if(p.getDistance(player) <= 1500)//玩家與擊殺玩家直線距離500內才會給予物品，以防城內掛機得物品
							{
								if(p == playerLeader)
								{
									ItemFunctions.addItem(p, itembos, 1, true);//給物品 聯軍長給2個
								}
								else
								{
									ItemFunctions.addItem(p, itembos, 1, true);//給物品 一般玩家給一個
								}
/* 								p.sendPacket(new SayPacket2(p.getObjectId(), ChatType.BLUE_UNK, "BLUE_UNK", "成功击杀首领，获得狩猎礼盒。")); */
							}
						}
					}
				}
					...
					for(Player p : player.getParty())
					{
						if((p.getLevel() < MaxLevel) && (p.getLevel() > MinLevel))//判斷所有的玩家比較玩家跟BOSS的等級差再給物品。
						{
							if(p.getDistance(player) <= 1500)//玩家與擊殺玩家直線距離500內才會給予物品，以防城內掛機得物品
							{
								if(p == playerLeader)
								{
									ItemFunctions.addItem(p, itembos, 1, true);//給物品 隊長長給2個									
								}
								else
								{
									ItemFunctions.addItem(p, itembos, 1, true);//給物品 一般玩家給一個
			...
			{
				//這裡是單人擊殺的情況如果不要註解掉就行了
				if((player.getLevel() < MaxLevel) && (player.getLevel() > MinLevel)) 
				{
					ItemFunctions.addItem(player, itembos, 1, true);//給物品
					/* player.sendPacket(new SayPacket2(player.getObjectId(), ChatType.BLUE_UNK, "BLUE_UNK", "成功击杀首领，获得狩猎礼盒。")); */
		...
		if(_spawnDeathKnight && !isBoss() && getReflection().isMain() && Rnd.chance(10))
		{
			...
			if(getLevel() >= 20 && getLevel() < 30)
				...
				knightId = 25791;
			else if(getLevel() >= 70 && getLevel() < 86)//86級還會出死亡騎士
				knightId = 25792;

			if(knightId > 0)
			{
				NpcInstance npc = NpcUtils.spawnSingle(knightId, getLoc(), getReflection(), 900000L); // TODO: Проверить процесс деспавна на оффе.

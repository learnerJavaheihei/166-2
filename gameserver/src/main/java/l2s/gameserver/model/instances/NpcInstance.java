...
import l2s.gameserver.instancemanager.DimensionalRiftManager;//夢幻結界
...
import l2s.gameserver.model.entity.DimensionalRift;//夢幻結界
...
public class NpcInstance extends Creature
{
	...
	public NpcInstance(int objectId, NpcTemplate template, MultiValueSet<String> set)
	{
		...
		for(Skill skill : template.getSkills().valueCollection())
			addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, skill));
		...
		_immobilized = getParameter("is_immobilized", false);
		deathImmune = getParameter("death_immune", false);
	}
	...
	{
		return getLeader() != null && isSpawnLeaderDepends() ? getLeader().getLoc() : _spawnedLoc;
	}
	...
	protected void onReduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp, boolean isDot)
	{
		...
		if(attacker.isConfused())
		{
			for(Abnormal abnormal : attacker.getAbnormalList())
			{
				...
				{
					if(effect.getName().equalsIgnoreCase("Discord"))
					{
		...
		getAggroList().addDamageHate(damager, (int) Math.min(damage, getCurrentHp()), 0);
	...
	{
		_masterRef = leader == null ? null : leader.getRef();
	}

	public NpcInstance getLeader()
	{
		return _masterRef == null ? null : _masterRef.get();
	}

	@Override
	public boolean isMinion()
	{
		return _masterRef != null;
	}
	...
	public Location getRndMinionPosition()
	{
		...
		final int minRadius = (int) getCurrentCollisionRadius() + 30;

		for(int i = 0; i < 100; i++)
		{
			int x = Rnd.get(minRadius * 2, offset * 2); // x
			int y = Rnd.get(x, offset * 2); // distance
			y = (int) Math.sqrt((y * y) - (x * x)); // y
			if(x > (offset + minRadius))
				x = (getX() + x) - offset;
			else
				x = (getX() - x) + minRadius;

			if(y > (offset + minRadius))
				y = (getY() + y) - offset;
			else
				y = (getY() - y) + minRadius;

			int tempz = GeoEngine.getLowerHeight(x, y, getZ(), getGeoIndex());
			if(Math.abs(getZ() - tempz) < 200 && GeoEngine.getLowerNSWE(x, y, tempz, getGeoIndex()) == GeoEngine.NSWE_ALL)
				return new Location(x, y, tempz);
		}
		return Location.findPointToStay(getX(), getY(), getZ(), offset, offset, getGeoIndex());
	}
	...
	protected void onSpawn()
	{
		...
		if(!hasMinions()) // Если миньоны были добавлены спавном, или в скриптах, то базовых миньонов не добавляем.
		{
		...
		if(hasMinions() && !isArenaRaid())
			ThreadPoolManager.getInstance().schedule(() -> getMinionList().spawnMinions(), 1500L); // DS: AI мастера должен запуститься раньше спавна миньонов.
	...
	{
		return (long) getStat().calc(Stats.EXP_RATE_MULTIPLIER, getTemplate().rewardExp, null, null);
	}

	public long getSpReward()
	{
		return (long) getStat().calc(Stats.SP_RATE_MULTIPLIER, getTemplate().rewardSp, null, null);
	}
	...
	public void onMenuSelect(Player player, int ask, long reply, int state)
	{
		for(QuestState qs : player.getAllQuestsStates())
		{
			...
			{
				if(qs.getQuest().notifyMenuSelect((int) reply, qs, this))
					return;
				break;
			}
		}
		...
		if(ask == -303)
		{
			...
			MultiSellHolder.getInstance().SeparateAndSend((int) reply, player, castle != null ? castle.getSellTaxRate() : 0);
		}
		else if(ask == -1000)
		{
			if(reply == 1)
		...
		try
		{
			...
			else if(command.startsWith("NpcLocationInfo"))
			{
				...
				if(!npcs.isEmpty())
				{
					...
					player.sendPacket(new RadarControlPacket(2, 2, npcs.get(0).getLoc()));
					// Ставим флажок на карте и стрелку на компасе
					player.sendPacket(new RadarControlPacket(0, 1, npcs.get(0).getLoc()));
				}
			}
			...
			else if(command.startsWith("EnterRift"))
			{
				...
				st2.nextToken();
				Integer b1 = Integer.valueOf(Integer.parseInt(st2.nextToken()));
			...
			else if(cmd.startsWith("Augment"))
			{
	...
	{
		showChatWindow(player, getHtmlPath(getHtmlFilename(0, player), player), firstTalk, replace);
	}
	...
	public void showChatWindow(Player player, int val, boolean firstTalk, Object... replace)
	{
		if(val == 0)
		{
			showMainChatWindow(player, firstTalk, replace);
			return;
		}
		showChatWindow(player, getHtmlPath(getHtmlFilename(val, player), player), firstTalk, replace);
	}
	...
	public String getHtmlDir(String filename, Player player)
	{
		...
		{
			if(HtmCache.getInstance().getIfExists(getTemplate().getHtmRoot() + filename, player) != null)
	...
	//認證技能--
	public static void showAdventurerSkillList(Player player) ///*9*/ADVENTURER,
	{
		showAcquireList(AcquireType.ADVENTURER, player);
	}
	public static void showForgottenPowerSkillList(Player player) ///*10*/FORGOTTENPOWER,
	{
		showAcquireList(AcquireType.FORGOTTENPOWER, player);
	}
	//--遺忘之力
	...
	{
		return _spawnLeaderDepends;
	}

	public void setSpawnLeaderDepends(boolean value)
	{
		_spawnLeaderDepends = value;
	}
	...
	public void onAdventurerSkillBypass(Player player)//認證技能--
	{
		//
	}
	public void onForgottenPowerSkillBypass(Player player)
	{
		//
	}
	//--遺忘之力
	...
	public int getLifeTime()
	{
		...
		return (int) ((System.currentTimeMillis() - _spawnTime) / 1000);
	}

	public boolean inMyTerritory(Creature creature)
	{
		if(getLeader() != null)
			return getLeader().inMyTerritory(creature);

		SpawnRange spawnRange = getSpawnRange();
	...
	{
		if(isVisible()) // Хозяин заспавнен.
		{
			MinionSpawner spawner = getMinionList().addMinion(npcId, ai, 1, respawn);
	...
	public NpcInstance createOnePrivateEx(int npcId, String ai, int weight, int respawn, int x, int y, int z, int h, long p1, long p2, long p3)
	{
		if(isVisible()) // Хозяин заспавнен.
		{
			//MinionSpawner spawner = getMinionList().addMinion(npcId, ai, 1, respawn, x, y, z, h);
			//return spawner.spawnOne();
		}
		return null;
	}
	...
	{
		if(_rewardLists == null || _rewardLists.isEmpty())
			return getTemplate().getRewards();

		List<RewardList> rewardLists = new ArrayList<RewardList>(getTemplate().getRewards().size() + _rewardLists.size());
		rewardLists.addAll(getTemplate().getRewards());
		rewardLists.addAll(_rewardLists);
		return rewardLists;
	}

	public void addRewardList(RewardList list)
	{
		if(_rewardLists == null)
			_rewardLists = new ArrayList<RewardList>();
		_rewardLists.add(list);
	}

	public void removeRewardList(RewardList list)
	{
		if(_rewardLists != null)
			_rewardLists.remove(list);
	}
	...
	{
		return _immobilized || super.isImmobilized();
	}
	...
	public boolean isDeathImmune() {
		return deathImmune || super.isDeathImmune();
	}

...
import l2s.gameserver.model.items.ItemInstance;//密封結界護身符和墜飾
...
import l2s.gameserver.utils.ItemFunctions;//密封結界護身符和墜飾
...
public class MerchantInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(actualCommand.equalsIgnoreCase("chat"))//密封結界護身符和墜飾--
		{
			try
			{
				if(st.countTokens() < 1)
		...
		else if(actualCommand.equalsIgnoreCase("Change"))
		{
			if(st.countTokens() < 1)
				return;
			...
			switch(reply)
			{
				case 814:
					needed_id = 70298;
					needed_money = 166320000;
					give_id = 71068;
					break;
				case 815:
					needed_id = 29708;
					needed_money = 727650000;
					give_id = 71074;
					break;
				case 816:
					needed_id = 29713;
			...
			player.getInventory().getItemByItemId(needed_id);
			
			ItemInstance item = player.getInventory().getItemByItemId(needed_id);
			if(item == null )
			{
				showChatWindow(player, "default/32478-noitem.htm", true);
				return;
			}
			if( item.getEnchantLevel()!= 10)
			{
				showChatWindow(player, "default/32478-nolevel.htm", true);
				return;
			}
			 if(player.getInventory().destroyItemByItemId(57, needed_money))
			 {
				ItemFunctions.deleteItem(player, item, 1, true);
				ItemFunctions.addItem(player, give_id, 1,true);
			 }
		}//--密封結界護身符和墜飾
		else
	...
	//認證技能--
	@Override
	public void onAdventurerSkillBypass(Player player)
	{
		showAdventurerSkillList(player);
	}
	@Override
	public void onForgottenPowerSkillBypass(Player player)
	{
		
		showForgottenPowerSkillList(player);
	}
	//--遺忘之力
}
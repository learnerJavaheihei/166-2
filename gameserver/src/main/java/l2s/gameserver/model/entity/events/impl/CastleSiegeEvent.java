...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
	...
	public boolean canRegisterOnSiege(Player player, Clan clan, boolean attacker)
	{
		if(attacker)
		{
			...
			if(getSiegeClan(CastleSiegeEvent.ATTACKERS, clan) != null)
			{
				player.sendPacket(SystemMsg.YOU_HAVE_ALREADY_REQUESTED_A_CASTLE_SIEGE);
				return false;
			}

			if(getSiegeClan(CastleSiegeEvent.DEFENDERS, clan) != null || getSiegeClan(CastleSiegeEvent.DEFENDERS_WAITING, clan) != null || getSiegeClan(CastleSiegeEvent.DEFENDERS_REFUSED, clan) != null)
			{

...
public class AuctionManager
{
	...
	private boolean checkIfItsOk(Player seller, ItemInstance item, long quantity, long salePrice)
	{
		...
		if(item == null)
		{
			sendMessage(seller, "您试图出售的物品不存在！");
			return false;
		}
		...
		if(item.isEquipped())
		{
			sendMessage(seller, "你需要先取消该项目！");
			return false;
		}
		if(item.isAugmented())
		{
			sendMessage(seller, "你不能出售强化武器！");
			return false;
		}
		if(item.getTemplate().isQuest())
		{
			sendMessage(seller, "你不能出售任务物品！");
			return false;
		}
		if(!item.canBeTraded(seller))
		{
			sendMessage(seller, "您不能出售此商品！");
			return false;
		}
		if((!seller.getServitors().isEmpty()) && (item.getItemType() == EtcItemType.PET_COLLAR))
		{
			sendMessage(seller, "在尝试出售此物品之前，请取消您的宠物。");
			return false;
		}
		if((!seller.getServitors().isEmpty()) && item.isSummon() && item.getTemplate().isForPet())
		{
			sendMessage(seller, "在尝试出售此物品之前，请取消您的宠物。");
			return false;
		}
		if(quantity < 1)
		{
			sendMessage(seller, "数量太低！");
			return false;
		}
		if(item.getCount() < quantity)
		{
			sendMessage(seller, "你没有足够的物品出售！");
			return false;
		}
		if(seller.getAdena() < Config.AUCTION_FEE)
		{
			sendMessage(seller, "你没有足够的钱来支付费用！");
			return false;
		}
		if(salePrice <= 0)
		{
			sendMessage(seller, "销售价格太低！");
			return false;
		}
		if(salePrice > 999999999999L)
		{
			sendMessage(seller, "价格太高了！");
			return false;
		}
		if(seller.isBlocked())
		{
			sendMessage(seller, "被阻止时无法创建拍卖！");
			return false;
		}
		if(getMyAuctions(seller).size() >= 10)
		{
			sendMessage(seller, "你一次只能有10次拍卖！");
			return false;
		}
		if(!Config.ALLOW_AUCTION_OUTSIDE_TOWN && !seller.isInPeaceZone())
		{
			sendMessage(seller, "您不能在城外添加新的拍卖！");
			return false;
		}
		if(seller.isInStoreMode())
		{
			sendMessage(seller, "在创建新拍卖之前关闭您的店铺！");
			return false;
		}
		if(_lastMadeAuction.containsKey(seller.getObjectId()))
		{
			...
			{
				sendMessage(seller, "两次上架间隔时间太短，请稍后再试，或者关闭交易行窗口再次打开!");
				return false;
			}
		}
		return true;

		...
		for(InstantZone.SpawnInfo s : si)
		{
			switch(s.getSpawnType())
			{
				case 0: // точечный спаун, в каждой указанной точке
					...
					{
						addSpawn(NpcUtils.spawnSimple(s.getNpcId(), loc, this, 1, s.getRespawnDelay(), s.getRespawnRnd(), /*TODO*/null));
					}
					break;
				case 1: // один точечный спаун в рандомной точке
					addSpawn(NpcUtils.spawnSimple(s.getNpcId(), new LocationsList(s.getCoords()), this, 1, s.getRespawnDelay(), s.getRespawnRnd(), /*TODO*/null));
					break;
				case 2: // локационный спаун
					addSpawn(NpcUtils.spawnSimple(s.getNpcId(), s.getLoc(), this, s.getCount(), s.getRespawnDelay(), s.getRespawnRnd(), /*TODO*/null));
					break;
			}
		}
	}

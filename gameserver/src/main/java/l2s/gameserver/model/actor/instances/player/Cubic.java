...
public class Cubic implements Runnable
{
	...
	private boolean doCastSkill(final CubicSkillInfo info)
	{
		if(isSkillDisabled(info))
			return false;

		final Skill skill = info.getSkill();
		final CubicTargetType targetType = getTargetType(info);

		Creature target = targetType.getTarget(this, skill);
		if(target == null)
			return false;

		if(target.isDead() || (target.isDoor() && !info.isCanAttackDoor()) || !canCastSkill(target, skill) || skill.isOffensive() && !target.isAutoAttackable(_owner))
			return false;
		...
		{
			case HEAL:
				chance = info.getChance((int) target.getCurrentHpPercents());
				break;
			case MANA_HEAL:
				chance = info.getChance((int) target.getCurrentMpPercents());
				break;
			default:
				chance = info.getChance();
				break;
		}
		...
		if(!skill.isNotBroadcastable())
			_owner.broadcastPacket(new MagicSkillUse(_owner, target, skill.getDisplayId(), skill.getDisplayLevel(), 0, 0));
		...
		_castTask = ThreadPoolManager.getInstance().schedule(() ->
		{
			...
			if(skill.isOffensive() && aimTarget.isNpc())
			{
				...
				else
				{
					...
					aimTarget.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED, _owner, skill, damage);
				}
			}
			_castTask = null;
		}, hitTime);
	...
	public boolean canCastSkill(Creature target, Skill skill)
	{
		if(!GeoEngine.canSeeTarget(_owner, target)) // Для кубиков это обязательное условие.
			return false;

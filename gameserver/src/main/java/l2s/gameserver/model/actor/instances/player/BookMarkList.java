...
{
	private static final int TELEPORT_FLAG_ID = 20033;//自由傳送旗
	private static final int[] TELEPORT_SCROLLS = { 13016/**, 13302, 20025**/ };//自由傳送卷軸
...
//
//			statement = con.prepareStatement("UPDATE `character_bookmarks` SET name=? AND acronym=? AND icon=? AND x=? AND Y=? AND Z=? WHERE char_Id=? AND idx=?");
//			for(int i = 0; i <elementData.size(); i++)
//			{
//				statement.setString(1, elementData.get(i).getName());
//				statement.setString(2, elementData.get(i).getAcronym());
//				statement.setInt(3, elementData.get(i).getIcon());
//				statement.setInt(4, elementData.get(i).x);
//				statement.setInt(5, elementData.get(i).y);
//				statement.setInt(6, elementData.get(i).z);
//				statement.setInt(7, owner.getObjectId());
//				statement.setInt(8, i+1);

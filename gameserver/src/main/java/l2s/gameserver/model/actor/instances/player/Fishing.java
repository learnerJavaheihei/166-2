...
public class Fishing
{
		...
		public void run()
		{
			...
			else
			{
				...
				if(rewards != null)
				{
					...
					exp *= _owner.getFishingExpMod();//釣魚技能EXP/SP實裝
					sp *= _owner.getFishingSpMod();//釣魚技能EXP/SP實裝
	...
	private boolean throwHook(boolean start, Location hookLoc)
	{
		...
		_inProcess = true;
		_processTask = ThreadPoolManager.getInstance().schedule(new FishingTask(fish), (long) (fish.getDuration() * _rod.getDurationModifier() * 1000L));
		return true;
	}
	...
	public Location findHookLocation()
	{
		...
		int baitZ = _owner.getZ() + ((int) _owner.getCurrentCollisionHeight() * 2) + 50;

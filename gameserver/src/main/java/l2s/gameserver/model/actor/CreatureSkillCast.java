...
public class CreatureSkillCast
{
	...
	private void onMagicUseTimer()
	{
		...
		if(skill.getCastRange() != -1 && skill.getSkillType() != SkillType.TAKECASTLE && !GeoEngine.canSeeTarget(_actor, aimingTarget))
		{
		...
		if(skill.getCondCharges() > 0 && _actor.getIncreasedForce() > 0)
		{
			int decreasedForce = skill.getCondCharges();
			if(decreasedForce > 15)
				decreasedForce = 5;
			_actor.setIncreasedForce(_actor.getIncreasedForce() - decreasedForce);
		}

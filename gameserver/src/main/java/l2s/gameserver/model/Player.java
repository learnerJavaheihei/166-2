...
public final class Player extends Playable implements PlayerGroup
{
	...
	public static final Location STABLE_LOCATION = new Location(82757, 148689, -3456);//奧林匹亞競賽中小退後位置改變
	...
	private int _accessorySetEnchant = 0;//20191223新增的
	...
	public synchronized final void setTransform(TransformTemplate transform)
	{
		...
		sendPacket(new FlyToLocationPacket(this, destLoc, FlyToLocationPacket.FlyType.DUMMY, 0, 0, 0));
		setLoc(destLoc);
	...
	public void refreshExpertisePenalty()
	{
		...
		if(expertiseIndex > 0)
		{
			for(; expertiseIndex >= 1; expertiseIndex--)
			{
		...
		if(noRateExp > 0)
		{
			if(!(getVarBoolean("NoExp") && getExp() == Experience.getExpForLevel(getLevel() + 1) - 1))
			{
				...
				if(elemental != null)
				{
					...
					long elementalExp = (long) (noRateExp / 281. * (Config.RATE_ELEMENTAL_XP + (mulRate - 1.))); // TODO: Check formula.
					if(elementalExp > 0)
	...
	public final void doPurePk(final Player killer)
	{
		...
		//if (killer.isGM())//GM殺人屏掉
		if (killer.isGM()|| isGM())//GM殺人屏掉 或 GM被殺 都屏掉 		{
		{
			return;
		}
		//殺人訊息只能在這裡控制了，原始config裡的已不再使用了。
		String pkmsg_TW = "公告：$killer 殘忍地屠殺了 $target";
		String pkmsg = "公告：$killer 残忍地屠杀了 $target";
		
		pkmsg_TW = pkmsg_TW.replace("$killer", killer.getName()).replace("$target", getName());
		pkmsg = pkmsg.replace("$killer", killer.getName()).replace("$target", getName());

		SayPacket2 CHINESE_TW = new SayPacket2(0, ChatType.ANNOUNCEMENT, "", pkmsg_TW);
		SayPacket2 CHINESE = new SayPacket2(0, ChatType.ANNOUNCEMENT, "", pkmsg);

		//這裡只能自己定義怎麼公告了。
		/* for(Player player : GameObjectsStorage.getPlayers(false, false))//20190724全屏公告PK
		{
			if(player.getLanguage() == Language.CHINESE_TW)
			{
				player.sendPacket(CHINESE_TW);
			}
			else
			{
				player.sendPacket(CHINESE);
			}
		}	
		//Announcements.announceToAll(pkmsg);//20190724全屏公告PK	 */	
	}
	...
	protected void doPKPVPManage(Creature killer)
	{
		...
		if(killer.isPlayer() || killer instanceof FakePlayer) //addon if killer is clone instance should do also this method.
		{
			...
			if(getPvpFlag() > 0 || war || castle != null && castle.getResidenceSide() == ResidenceSide.DARK)
			{
				...
				//if (killer.isGM())//GM殺人屏掉
				if (pk.isGM()|| isGM())//GM殺人屏掉 或 GM被殺 都屏掉 		{
				{
					return;
				}
				//殺人訊息只能在這裡控制了，原始config裡的已不再使用了。
				String pkmsg_TW = "公告：$killer 戰勝 $target";
				String pkmsg = "公告：$killer 战胜 $target";
				pkmsg_TW = pkmsg_TW.replace("$killer", pk.getName()).replace("$target", getName());
				pkmsg = pkmsg.replace("$killer", pk.getName()).replace("$target", getName());
				...
				//String pvpmsg = Config.ANNOUNCE_PVP_MSG.replace("$killer", pk.getName()).replace("$target", getName());
				//Announcements.announceToAll(pvpmsg);//20190724全屏公告PVP */
			}
			else
			{
				doKillInPeace(pk);
		...
		try
		{
			...
			{
				//判斷物品是否可以掉落  另一個是有性向值不可掉落的清單(KARMA_LIST_NONDROPPABLE_ITEMS) 這二類型不可以掉落
			...
			checkAddItemToDrop(drop, dropWeapon, dropWeaponCount);//設置掉落武器的數量
			checkAddItemToDrop(drop, dropEquip, dropEquipCount);//設置掉落裝備的數量
			checkAddItemToDrop(drop, dropItem, dropItemCount);//設置掉落道具的數量
			...
			{
				//如果物品是精鍊過的 而且 精鍊設置可以掉落
				//if(item.isAugmented() && !Config.ALT_ALLOW_DROP_AUGMENTED)
				//{
					//item.setVariationStoneId(0);//移除外觀
					//item.setVariation1Id(0);//移除精鍊
					//item.setVariation2Id(0);//移除精鍊
				//}

				item = getInventory().removeItem(item);//掉落者移除這一件物品
				...
				//殺人者是玩家 而且設置自動掉落且設置當pk時自動掉落 或者是在空中時候
				if(killer.isPlayable() && ((Config.AUTO_LOOT && Config.AUTO_LOOT_PK) || isInFlyingTransform()))
				{
					killer.getPlayer().getInventory().addItem(item);//殺人者 獲取這一件物品，就是掉落者掉落的物品
	...
	private void restoreSkills()
	{
		...
		try
		{
			...
			if(Config.UNSTUCK_SKILL && getSkillLevel(1050) < 3)//修改出生回歸技能
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 2213, 10));//修改出生回歸技能
	...
	{
		//修改默認的語言return getLanguage() == Language.RUSSIAN || getLanguage().getSecondLanguage() == Language.RUSSIAN;
		return getLanguage() == Language.CHINESE_TW || getLanguage().getSecondLanguage() == Language.CHINESE_TW;//修改默認台灣繁體
	}
	...
	public void reviveRequest(Player reviver, double percent, boolean pet)
	{
		...
		if(pet && getPet() != null && getPet().isDead() || !pet && isDead())
		{
			...
			/*内挂復活配套--*/
			if(_isInPlugIn)//如果自身处于开内挂时需要先判断是否是对方是同队的人才产生自行点击复活
			{
	...
	//新增釣魚exp
	public double getFishingExpMod()
	{
		double mod = 1;
		mod *= 1. + getStat().calc(Stats.FISHING_EXP_MULTIPLIER, 0, null, null);
		return mod;
	}
	public double getFishingSpMod()
	{
		double mod = 1;
		mod *= 1. + getStat().calc(Stats.FISHING_SP_MULTIPLIER, 0, null, null);
		return mod;
	}
	//新增釣魚SP
	...
	{
		return _keyBindings;
		//return ArrayUtils.EMPTY_BYTE_ARRAY; //20190808 暫時移除這一個功能
	}
	...
	public void setKeyBindings(byte[] keyBindings)
	{
		if(keyBindings == null)
			...
			_keyBindings = keyBindings;//20190808 暫時移除這一個功能
	}
	...
	{
		return getStat().calc(Stats.ACTIVE_AGATHION_MAIN_SLOT, 0, null, null) > 0;
	}

	public int getSubAgathionsLimit()
	{
		return (int) getStat().calc(Stats.SUB_AGATHIONS_LIMIT, 0, null, null);
	}
	...
	public final String getVisibleTitle(Player receiver)
	{
		...
		if(isInAwayingMode())
		{
			...
			if(awayText == null || awayText.length() <= 1)
				return isLangRus() ? "<Отошел>" : "<暫離>";
			else
				return (isLangRus() ? "<Отошел>" : "<暂离>") + " - " + awayText + "*";
		}
	...
	public void checkLevelUpReward(boolean onRestore)
	{
		...
		if(playerLvl > lastRewardedByClass)
		{
			for(int i = playerLvl; i > lastRewardedByClass; i--)
			{
				if(getClan() != null && getClan().getLevel() >= 3)
				{
					...
					switch(i)
					{
						case 20: earnedPoints = 2; break;
						case 21: earnedPoints = 2; break;
						case 22: earnedPoints = 2; break;
						case 23: earnedPoints = 2; break;
						case 24: earnedPoints = 2; break;
						case 25: earnedPoints = 2; break;
						case 26: earnedPoints = 4; break;
						case 27: earnedPoints = 4; break;
						case 28: earnedPoints = 4; break;
						case 29: earnedPoints = 4; break;
						case 30: earnedPoints = 4; break;
						case 31: earnedPoints = 6; break;
						case 32: earnedPoints = 6; break;
						case 33: earnedPoints = 6; break;
						case 34: earnedPoints = 6; break;
						case 35: earnedPoints = 6; break;
						case 36: earnedPoints = 8; break;
						case 37: earnedPoints = 8; break;
						case 38: earnedPoints = 8; break;
						case 39: earnedPoints = 8; break;
						case 40: earnedPoints = 8; break;
						case 41: earnedPoints = 10; break;
						case 42: earnedPoints = 10; break;
						case 43: earnedPoints = 10; break;
						case 44: earnedPoints = 10; break;
						case 45: earnedPoints = 10; break;
						case 46: earnedPoints = 12; break;
						case 47: earnedPoints = 12; break;
						case 48: earnedPoints = 12; break;
						case 49: earnedPoints = 12; break;
						case 50: earnedPoints = 12; break;
						case 51: earnedPoints = 14; break;
						case 52: earnedPoints = 14; break;
						case 53: earnedPoints = 14; break;
						case 54: earnedPoints = 14; break;
						case 55: earnedPoints = 14; break;
						case 56: earnedPoints = 16; break;
						case 57: earnedPoints = 16; break;
						case 58: earnedPoints = 16; break;
						case 59: earnedPoints = 16; break;
						case 60: earnedPoints = 16; break;
						case 61: earnedPoints = 18; break;
						case 62: earnedPoints = 18; break;
						case 63: earnedPoints = 18; break;
						case 64: earnedPoints = 18; break;
						case 65: earnedPoints = 18; break;
						case 66: earnedPoints = 21; break;
						case 67: earnedPoints = 21; break;
						case 68: earnedPoints = 21; break;
						case 69: earnedPoints = 21; break;
						case 70: earnedPoints = 21; break;
						case 71: earnedPoints = 25; break;
						case 72: earnedPoints = 25; break;
						case 73: earnedPoints = 25; break;
						case 74: earnedPoints = 25; break;
						case 75: earnedPoints = 25; break;
					}
		...
		/* add ExpManagerTime edit by cy 02/20/2023 */
		CheckAddExpEffect();
		/* fin */
	...
	/* add ExpManagerTime edit by cy 02/20/2023 */	
	private void CheckAddExpEffect() {
		
		int TopLv = AddExpManager.TopLv;
		int lv = getLevel();
		int lvgap = 0;
		String str = null;
		...
		switch (lvgap) {
			case 0: case 1: case 2: case 3:
				// no exp bonus
				removeSkillById(46016);
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 無經驗加成效果。";
				break;
			 case 4: case 5: case 6:
				// skill 1
				/* player.altUseSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 55000, 1), player); */
				removeSkillById(46016);
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 46016, 1));
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 1階經驗加成效果。";
				break;
			 case 7: case 8: case 9: case 10: case 11: case 12: case 13: case 14: case 15:
				// skill 2
				/* player.altUseSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 55000, 1), player); */
				removeSkillById(46016);
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 46016, 2));
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 2階經驗加成效果。";
				break;
			case 16: case 17: case 18: case 19:	case 20: case 21: case 22: case 23: case 24:		
				// skill 3
				/* player.altUseSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 55000, 1), player); */
				removeSkillById(46016);
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 46016, 3));
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 3階經驗加成效果。";
				break;
			case 25: case 26: case 27: case 28:	case 29: case 30: case 31: case 32: case 33: case 34: case 35:
				// skill 4
				/* player.altUseSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 55000, 1), player); */
				removeSkillById(46016);
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 46016, 4));
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 4階經驗加成效果。";
				break;
			default :
				// skill 5
				/* player.altUseSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 55000, 1), player); */
				removeSkillById(46016);
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 46016, 5));
				str = "您目前與等於排行第一玩家等級相距: " + lvgap + " , 5階經驗加成效果。";
				break;
			}
		SayPacket2 cs = new SayPacket2(0, ChatType.ANNOUNCEMENT, getName(), str);
		sendPacket(cs);
	}
	/* fin */
	...
	//20191223新增的
	public int getAccessorySetEnchant()
	{
		return _accessorySetEnchant;
	}
	//20191223新增的
	public void setAccessorySetEnchant(int value)
	{
		_accessorySetEnchant = value;
	}
	...
	public SkillEntry getAdditionalSSEffect(boolean spiritshot, boolean blessed)
	{
		if(!spiritshot)
		{
			...
			{
				// 普通彈
				if(ItemFunctions.checkIsEquipped(this, -1, 71372, 0)) // 高級紅寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 71371, 0)) // 高級紅寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 71370, 0)) // 高級紅寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 7);
				if(ItemFunctions.checkIsEquipped(this, -1, 71369, 0)) // 高級紅寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 5);
				if(ItemFunctions.checkIsEquipped(this, -1, 71368, 0)) // 高級紅寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 3);
				if(ItemFunctions.checkIsEquipped(this, -1, 70455, 0)) // 紅寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70454, 0)) // 紅寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17816, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70769, 0)) // 紅寶石 - Lv4  限時7日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17816, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70453, 0)) // 紅寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17815, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70452, 0)) // 紅寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17814, 2);
				if(ItemFunctions.checkIsEquipped(this, -1, 70451, 0)) // 紅寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17814, 1);
				// 第2組
				if(ItemFunctions.checkIsEquipped(this, -1, 91324, 0)) // 高級紅寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91431, 0)) // 高級紅寶石 - Lv5 限時30日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91323, 0)) // 高級紅寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91322, 0)) // 高級紅寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 7);
				if(ItemFunctions.checkIsEquipped(this, -1, 91321, 0)) // 高級紅寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 5);
				if(ItemFunctions.checkIsEquipped(this, -1, 91320, 0)) // 高級紅寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 3);
				if(ItemFunctions.checkIsEquipped(this, -1, 90332, 0)) // 紅寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 91418, 0)) // 紅寶石 - Lv5 限時30日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90331, 0)) // 紅寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17816, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90330, 0)) // 紅寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17815, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90329, 0)) // 紅寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17814, 2);
				if(ItemFunctions.checkIsEquipped(this, -1, 90328, 0)) // 紅寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17814, 1);
			}
			else
			{
				// 祝福彈
				if(ItemFunctions.checkIsEquipped(this, -1, 71372, 0)) // 高級紅寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 10);
				if(ItemFunctions.checkIsEquipped(this, -1, 71371, 0)) // 高級紅寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 10);
				if(ItemFunctions.checkIsEquipped(this, -1, 71370, 0)) // 高級紅寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 8);
				if(ItemFunctions.checkIsEquipped(this, -1, 71369, 0)) // 高級紅寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 6);
				if(ItemFunctions.checkIsEquipped(this, -1, 71368, 0)) // 高級紅寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17817, 4);
				if(ItemFunctions.checkIsEquipped(this, -1, 70455, 0)) // 紅寶石 - Lv5
		...
		else
		{
			...
			{
				// 普通彈
				if(ItemFunctions.checkIsEquipped(this, -1, 71377, 0)) // 高級藍寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 71376, 0)) // 高級藍寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 71375, 0)) // 高級藍寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 7);
				if(ItemFunctions.checkIsEquipped(this, -1, 71374, 0)) // 高級藍寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 5);
				if(ItemFunctions.checkIsEquipped(this, -1, 71373, 0)) // 高級藍寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 3);
				if(ItemFunctions.checkIsEquipped(this, -1, 70460, 0)) // 藍寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70459, 0)) // 藍寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17820, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70770, 0)) // 藍寶石 - Lv4 限時7日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17820, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70458, 0)) // 藍寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17819, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 70457, 0)) // 藍寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17818, 2);
				if(ItemFunctions.checkIsEquipped(this, -1, 70456, 0)) // 藍寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17818, 1);
				// 第2組
				if(ItemFunctions.checkIsEquipped(this, -1, 91329, 0)) // 高級藍寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91432, 0)) // 高級藍寶石 - Lv5 限時30日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91328, 0)) // 高級藍寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 9);
				if(ItemFunctions.checkIsEquipped(this, -1, 91327, 0)) // 高級藍寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 7);
				if(ItemFunctions.checkIsEquipped(this, -1, 91326, 0)) // 高級藍寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 5);
				if(ItemFunctions.checkIsEquipped(this, -1, 91325, 0)) // 高級藍寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 3);
				if(ItemFunctions.checkIsEquipped(this, -1, 90337, 0)) // 藍寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 91419, 0)) // 藍寶石 - Lv5 限時30日
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90336, 0)) // 藍寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17820, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90335, 0)) // 藍寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17819, 1);
				if(ItemFunctions.checkIsEquipped(this, -1, 90334, 0)) // 藍寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17818, 2);
				if(ItemFunctions.checkIsEquipped(this, -1, 90333, 0)) // 藍寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17818, 1);
			}
			else
			{
				// 祝福彈
				if(ItemFunctions.checkIsEquipped(this, -1, 71377, 0)) // 高級藍寶石 - Lv5
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 10);
				if(ItemFunctions.checkIsEquipped(this, -1, 71376, 0)) // 高級藍寶石 - Lv4
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 10);
				if(ItemFunctions.checkIsEquipped(this, -1, 71375, 0)) // 高級藍寶石 - Lv3
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 8);
				if(ItemFunctions.checkIsEquipped(this, -1, 71374, 0)) // 高級藍寶石 - Lv2
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 6);
				if(ItemFunctions.checkIsEquipped(this, -1, 71373, 0)) // 高級藍寶石 - Lv1
					return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 17821, 4);
				if(ItemFunctions.checkIsEquipped(this, -1, 70460, 0)) // 藍寶石 - Lv5
	...
	public boolean canReceiveStatusUpdate(Creature creature, UpdateType updateType, int field)
	{
		...
		if(creature.isPlayable())
		{
			...
			boolean canReceiveHpMp = isRegenOrDamage;
	...
	private int powerSkill[] = {
			60011,//是 黃晶潛能石-物理攻擊
			60012,//是 黃晶潛能石-魔法攻擊
			60013,//是 黃晶潛能石-物理致命傷害
			60014,//是 黃晶潛能石-魔法致命傷害
			60021,//是 紅晶潛能石-HP/CP1
			60022,//是 紅晶潛能石-反傷
			60023,//是 紅晶潛能石-減反傷
			60031,//是 粉晶潛能石-物理防禦
			60032,//是 粉晶潛能石-魔法防禦
	...
	public void recalculateStone()
	{
		ItemInstance itemInstance = null;
		Map<String, Integer> maps = new HashMap<String, Integer>();  
		Map<String, Integer> maps_skill = new HashMap<String, Integer>(); 
		for (int i = 0 ;i<powerSkill.length;i++)
		{
			removeSkillById(powerSkill[i]);//先移除目前所有狀態中的技能
		}
		for (int i = 0 ;i<specialskill.length;i++)
		{
			removeSkillById(specialskill[i]);//先移除目前所有狀態中的技能 正面對決 ....
		}
		removeSkillById(11363);//移除傳送
		 
		for (int i = 0 ;i<skill.length;i++)
		{
			removeSkillById(skill[i]);//先移除目前所有狀態中的技能 巨力....
		}
		...
		{
			itemInstance = getInventory().getPaperdollItem(bodyPart[i]);//再判斷是否有裝備此件防具
			if(itemInstance != null)
			{
				if(itemInstance.isAugmented())
				{
					//11 01 1 0
					id1 = String.valueOf(itemInstance.getVariation1Id());
					//					int skill = 60000 + Integer.parseInt(id1.substring(0, 2) + id1.substring(5, 6));
					//					int skilllv = Integer.parseInt(id1.substring(2, 4));
					//					this.addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, skill, skilllv));
					String specialskills = id1.substring(4, 5);

					if(!specialskills.equals("0"))//這一些是正面對決這一類的技能
					{
						if(maps.containsKey(specialskills))
						{
							maps.put(specialskills, maps.get(specialskills) + 1);
						}
						else
						{
							maps.put(specialskills, 1);
						}
					}
					
					//1 00 00 00
					id2 = String.valueOf(itemInstance.getVariation2Id());
					String s1 = "";
					int i1 = 0;
					for(int j = 0; j < 3; j++)//這一類是巨力之類的技能
					{
						s1 = id2.substring(j * 2 + 1, j * 2 + 2);
						i1 = Integer.parseInt(id2.substring(j * 2 + 2, j * 2 + 3));
						if(!s1.equals("0"))
						{
							if(maps_skill.containsKey(s1))
							{
								maps_skill.put(s1, maps_skill.get(s1) + i1);
							}
							else
							{
								maps_skill.put(s1, i1);
							}
						}
					}
				}
		...
		Iterator<Entry<String, Integer>> i = maps.entrySet().iterator();
		while(i.hasNext())
		{
			Entry<String, Integer> entry1 =i.next();
			if(entry1.getValue() > 2)//給 正面對決
			{
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 60000 + Integer.parseInt(entry1.getKey()), Math.min(3, (int) Math.floor(entry1.getValue() / 3))));
				if(Integer.parseInt(entry1.getKey()) == 5)
				{
					addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 11363, Math.min(3, (int) Math.floor(entry1.getValue() / 3))));
				}
				sendSkillList();
			}
		}
		i = maps_skill.entrySet().iterator();
		while(i.hasNext())
		{
			Entry<String, Integer> entry1 = i.next();
			if(entry1.getValue() > 14)//給 巨力
			{
				addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 60005 + Integer.parseInt(entry1.getKey()), Math.min(5, (int) Math.floor(entry1.getValue() / 15))));
				sendSkillList();
			}
		}
	}

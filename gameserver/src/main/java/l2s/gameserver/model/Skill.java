...
public class Skill extends StatTemplate implements SkillInfo, Cloneable
{
	...
	public static enum SkillType
	{
		...
		UNLOCK(Unlock.class),
		TRANSFER(Transfer.class),//修復召喚即時傳送門
	...
	private boolean _isVisbleTargetAbnormal = Config.SHOW_TARGET_EFFECTS;
	...
	public SystemMsg checkTarget(SkillEntry skillEntry, Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first, boolean trigger)
	{
		...
		{
			if(isAoE() && isOffensive() && !GeoEngine.canSeeTarget(activeChar, target))
				return SystemMsg.CANNOT_SEE_TARGET;
		}
...
//				return target != null && target.isDead() == isCorpse() && !(target == activeChar && isOffensive()) && (!_isUndeadOnly || target.isUndead()) && activeChar.getPlayer().isInSameClan(target.getPlayer()) ? target : null;
	...
	private void addTargetsToList(SkillEntry skillEntry, Set<Creature> targets, Creature aimingTarget, Creature activeChar, boolean forceUse)
	{
		...
		else
		{
			...
			if(affectLimit == 0)
			{
				...
				if(isOffensive() && !activeChar.isRaid())
					affectLimit = 20;
				else
					affectLimit = 256;
			}
		...
		*/
		...
		for(Creature target : targets)
		{
			...
			if(!isToggle() && !isCubicSkill()) // TODO: [Bonux] Добавить условия, на какие баффы множитель не влияет.
				abnormalTimeModifier *= target.getStat().calc(isOffensive() ? Stats.DEBUFF_TIME_MODIFIER : Stats.BUFF_TIME_MODIFIER, null, null);
	...
	{
		return _isSelfDispellable && !hasEffect(EffectUseType.NORMAL, "Transformation") && !isToggle() && !isOffensive() && !isMusic();
	}
	...
	private final boolean checkCastTarget(Creature target)
	{
		...
		/*TODO: Переделать.
		if(isOffensive() && target.isInvulnerable())
		{
	...
	public final void onTickCast(Creature activeChar, Set<Creature> targets)
	{
		...
		if(attachedSkill != null)
		{
			...
			for(Creature target : targets)
			{
				...
				if(skill.hasEffects(EffectUseType.NORMAL) && skill.calcEffectsSuccess(activeChar, target, true))
				{
	...
	public void onEndCast(Creature activeChar, Set<Creature> targets)
	{
		...
		for(Creature target : targets)
		{
			...
			if(hasEffects(EffectUseType.NORMAL) && calcEffectsSuccess(activeChar, target, true))
			{
	...
	**/
	...
	public void setIsVisibleTargetAbnormal(boolean value)
	{
		// Для заглушки отображения кондишона требующий эффект у цели.
		_isVisbleTargetAbnormal = value;
	}

	public boolean isVisibleTargetAbnormal()
	{
		// Для заглушки отображения кондишона требующий эффект у цели.
		return _isVisbleTargetAbnormal;
	}
}
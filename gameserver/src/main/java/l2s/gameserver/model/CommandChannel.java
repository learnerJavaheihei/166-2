...
public class CommandChannel implements PlayerGroup
{
	...
	public static boolean checkAuthority(Player creator)
	{
		...
		//也就是當你有戰略指導書就可以開聯軍
		if(creator.getInventory().getItemByItemId(STRATEGY_GUIDE_ID) == null)//戰略指導書
		{
			if(creator.getClan() !=null)//玩家必需要有盟
			{
				if(creator.getSkillLevel(CLAN_IMPERIUM_ID) <= 0)//盟誓 帝權授命 skill 或許這一個是要5級盟才可以學習的技能，故下面有提示5級的問題273行。
				{
					creator.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_AUTHORITY_TO_INVITE_SOMEONE_TO_THE_COMMAND_CHANNEL);//不具備聯合軍的邀請權限
					return false;
				}
			}
			else
			{
				//在這區表示你是隊長，但你不是某一個盟的盟主 
				//creator.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_AUTHORITY_TO_INVITE_SOMEONE_TO_THE_COMMAND_CHANNEL);
				return false;
			}
		}

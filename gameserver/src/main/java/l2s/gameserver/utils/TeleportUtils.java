...
public class TeleportUtils
{
	...
	public static TeleportPoint getRestartPoint(Player player, Location from, RestartType restartType)
	{
		...
		_log.warn("记录玩家出坐标出错 玩家: " + player.getName() + " " + player.getLoc().getX() + "," + player.getLoc().getY() + "," + player.getLoc().getZ() + "," + "!");

		if(player.getReflectionId() > 0)
		{
			_log.warn("玩家在即時地区(" + player.getActiveReflection().getInstancedZoneId() + " " + player.getActiveReflection().getName() + ")");
		}

...
public class TransactionBankDao {
	...
	public static double procedureRate ;
	...
	//查询手续费
    public void selectProcedureRate() {
        Connection con = null;
        PreparedStatement statement = null;
        try
        {
            con = DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement("SELECT procedure_rate FROM exchange_rate");
            ResultSet resultSet = statement.executeQuery();
            if(resultSet.next()){
                procedureRate = resultSet.getDouble(1);
            }
            DbUtils.close(statement);
        }
        catch(final SQLException e)

...
import l2s.gameserver.data.xml.holder.ItemHolder;/*製作裝備隨機強化*/
...
public class RequestRecipeShopMakeDo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(product != null)
		{
			...
			rate += manufacturer.getStat().calc(Stats.CRAFT_CHANCE_BONUS, 0);
			...
			if(Rnd.chance(rate))
			{
				...
				double craftcritical  = manufacturer.getStat().calc(Stats.CRAFT_CRITICAL_POWER,0); //53001製作暴擊--
				if(Rnd.chance(craftcritical))
				{
					ItemInstance item = ItemFunctions.createItem(itemId);
					if (item.isWeapon() || item.isArmor())
					{
						ItemFunctions.addItem(buyer, itemId, itemsCount, true);
						buyer.sendPacket(new SystemMessagePacket(SystemMsg.LADY_LUCK_SMILES_UPON_YOU));//幸運女神出現
					}
				}//--53001製作暴擊
				success = 1;
			}

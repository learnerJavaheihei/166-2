		...
		try
		{
				...
				else
				{
					...
					if(!template.isStackable())
						for(int i = 0; i < ingridientItemCount * _amount; i++)
						{
							...
							for (ItemInstance li : inventory.getItemsByItemId(ingridientItemId))//忽略身上裝備的物品
							{
								if(!li.isEquipped())
									list.add(li);
							}
							...
							else
							{
								...
								for(ItemInstance item : list)
								{//multisell支持enchant--
									if(list1.isKeepEnchant())
									{
										if(item.getEnchantLevel() < ingridientEnchant)
										{
											activeChar.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
											return;
										}
									...
									else
									{
										if(item.getEnchantLevel() != ingridientEnchant)
										{
											if (ingridientEnchant != 0)//當換的物品跟需求的等級不符合時的，//而且 這裡是多判定xml寫的是否帶強化，如果確定帶強化就真的讓他失敗 不給換 但如果是enchant=0情況時就讓他可換
											{
												activeChar.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);//原始
												return;//原始
											}
										}
									}
									if(item.isEquipped() )
									{
										activeChar.sendPacket(SystemMsg.THE_ITEM_THAT_IS_CURRENTLY_WORN_CANNOT_BE_REGISTERED);
										return;
									}//--multisell支持enchant
			...
			if(list1.getType() == MultisellType.CHANCED)
			{
				...
				if(Rnd.chance(chancesAmount))
				{
					...
					//原始移動到上面List<MultiSellIngredient> successProducts = new ArrayList<MultiSellIngredient>();
					int tryCount = 0;
			...
			for(MultiSellIngredient in : products)
			{
				if(in.getBouns() > 0)//multisell支持bouns--
				{
					if(!Rnd.chance(in.getBouns()))
					{
						continue;
					}
				}//--multisell支持bouns

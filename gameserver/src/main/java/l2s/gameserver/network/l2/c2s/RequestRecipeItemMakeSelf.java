...
import l2s.gameserver.model.Playable;/*製作裝備隨機強化*/
...
import l2s.gameserver.stats.Stats;//53002 製作大師
import l2s.gameserver.templates.item.EtcItemTemplate.EtcItemType;
import l2s.gameserver.templates.item.ItemTemplate;/*製作裝備隨機強化*/
...
public class RequestRecipeItemMakeSelf extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		rate +=activeChar.getStat().calc(Stats.CRAFT_CHANCE_BONUS,0);//53002 製作大師
		...
		if(product != null)
		{
			...
			if(Rnd.chance(rate))
			{
				...
				double craftcritical  = activeChar.getStat().calc(Stats.CRAFT_CRITICAL_POWER,0); //53001製作暴擊--
				if(Rnd.chance(craftcritical))//下面是爆擊了，多送一套
				{
					ItemInstance item = ItemFunctions.createItem(itemId);
					if (item.isWeapon() || item.isArmor())
					{
						ItemFunctions.addItem(activeChar, itemId, itemsCount, true);
						activeChar.sendPacket(new SystemMessagePacket(SystemMsg.LADY_LUCK_SMILES_UPON_YOU));//幸運女神出現
					}
				}//--53001製作暴擊
				success = 1;
			}

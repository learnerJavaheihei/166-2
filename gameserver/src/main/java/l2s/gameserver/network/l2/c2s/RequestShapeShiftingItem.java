...
import l2s.gameserver.utils.ItemFunctions;//外型加工石解除還加工石
		...
		try
		{
			...
			switch(appearanceStone.getType())
			{
				case RESTORE:
					...
					//外型加工石解除還加工石--
					int itemId = AppearanceStoneHolder.getInstance().getAppearanceStoneId(targetItem.getVisualId());
					if(itemId != 0)
					{
				...
				case FIXED:
					...
					if(appearanceStone.getPeriod() > 0)
					{
						...
						inventory.startLifeTask(targetItem);

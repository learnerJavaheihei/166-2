...
public class RequestEnchantItem extends L2GameClientPacket
{
	...
	private int _giveStoneCount;//強化失敗給予加工石
	private int _itemId;//強化失敗給予加工石
	//強化失敗給予加工石--
	private final int[] wapenD =
	{
		1,
		2,
		3,
	...
	protected void runImpl()
	{
		...
		try
		{
			...
			if(Rnd.chance(chance))
			{
				...
				player.sendPacket(new EnchantResultPacket(0, 0, 0, item.getEnchantLevel(), 0, 0));//強化失敗給予加工石
			...
			else
			{
				...
				switch(resultType)
				{
					case CRYSTALS:
						...
						//--強化失敗給予加工石
						if(item.getEnchantLevel() > 6 && item.isWeapon())
						{
							switch(item.getGrade())
							{
								case D:
									_giveStoneCount = wapenD[Math.min(item.getEnchantLevel() - 7, wapenD.length - 1)];
									break;
								case C:
									_giveStoneCount = wapenC[Math.min(item.getEnchantLevel() - 7, wapenC.length - 1)];
									break;
								case B:
									_giveStoneCount = wapenB[Math.min(item.getEnchantLevel() - 7, wapenB.length - 1)];
									break;
								case A:
									_giveStoneCount = wapenA[Math.min(item.getEnchantLevel() - 7, wapenA.length - 1)];
									break;
								case S:
									_giveStoneCount = wapenS[Math.min(item.getEnchantLevel() - 7, wapenS.length - 1)];
									break;
								default:
									break;
							}
							_itemId = 91462;
							ItemFunctions.addItem(player, _itemId, _giveStoneCount, true);
						}
						if(item.getEnchantLevel() > 5 && (item.isArmor()|| item.isAccessory()))
						{
							switch(item.getGrade())
							{
								case D:
									_giveStoneCount = aramD[Math.min(item.getEnchantLevel() - 6, aramD.length - 1)];
									break;
								case C:
									_giveStoneCount = aramC[Math.min(item.getEnchantLevel() - 6, aramC.length - 1)];
									break;
								case B:
									_giveStoneCount = aramB[Math.min(item.getEnchantLevel() - 6, aramB.length - 1)];
									break;
								case A:
									_giveStoneCount = aramA[Math.min(item.getEnchantLevel() - 6, aramA.length - 1)];
									break;
								case S:
									_giveStoneCount = aramS[Math.min(item.getEnchantLevel() - 6, aramS.length - 1)];
									break;
								default:
									break;
							}
							_giveStoneCount += (item.getEnchantLevel() - 6);
							_itemId = 91463;
							ItemFunctions.addItem(player, _itemId, _giveStoneCount, true);
						}
						//--強化失敗給予加工石
						...
						if(crystalId > 0 && item.getCrystalCountOnEchant() > 0 && !item.isFlagNoCrystallize())
						{
							...
							player.sendPacket(new EnchantResultPacket(1, crystalId, crystalAmount, 0, _itemId, _giveStoneCount));//強化失敗給予加工石
					...
					case DROP_ENCHANT:
						...
						break;
					case DROP_ENCHANT25:
						int enchantDropCount25 = enchantScroll.getEnchantDropCount();
						if(enchantStone != null && enchantStone.getEnchantDropCount() < enchantDropCount25)
							enchantDropCount25 = enchantStone.getEnchantDropCount();

						item.setEnchantLevel(Math.max(item.getEnchantLevel() - enchantDropCount25, 25));
						item.setJdbcState(JdbcEntityState.UPDATED);
						item.update();

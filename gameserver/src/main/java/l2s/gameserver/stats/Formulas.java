...
public class Formulas
{
	...
	public static AttackInfo calcMagicDam(Creature attacker, Creature target, Skill skill, double power, boolean useShot, boolean canMiss)
	{
		...
		//增加魔法技能迴避-技能ID 446
		if(info.damage > 0 && (skill.getSkillType() == SkillType.MDAM || skill.getSkillType() ==SkillType.DRAIN) && Rnd.chance(target.getStat().calc(Stats.M_SKILL_EVASION, 100, attacker, skill) - 100))
		{
			// @Rivelia. info.miss makes the Damage Text "Evaded" appear.
			info.miss = true;
			info.damage = 0;
	...
	*/
	...
	public static boolean calcPCrit(Creature attacker, Creature target, Skill skill, boolean blow)
	{
		if(skill != null)
		{
			...
			final double critRate = skillCritRate * skillCritChanceMod * pCritChanceMode * 10;
	...
	{
		if(skill.isMagic())
			return (int) (skillTime * 333 / Math.max(attacker.getMAtkSpd() * (attacker.getChargedSpiritshotPower() > 200 ? 1.1 : 1), 1));//祝福的魔靈彈增加施法速度
		if(skill.isPhysic())
	...
	public static boolean calcEffectsSuccess(Creature caster, Creature target, Skill skill, int activateRate)
	{
		...
		final double buffDebuffMod = 1. + ((skill.isOffensive() ? target.getStat().calc(Stats.RESIST_ABNORMAL_DEBUFF, 0) : target.getStat().calc(Stats.RESIST_ABNORMAL_BUFF, 0)) / 100.);

		...
		switch(stat)
		{
			...
			case MAGIC_ATTACK:
			{
				switch(item.getTemplate().getGrade().getCrystalId())
				{
					...
					case ItemTemplate.CRYSTAL_S:
						env.value += 4 * (enchant + overenchant * 4) * (isBlessed ? 1.6 : 1.0);//修復強化S級4倍
						break;
					case ItemTemplate.CRYSTAL_A:
						env.value += 3 * (enchant + overenchant * 2) * (isBlessed ? 1.6 : 1.0);//修復強級2倍
						break;
					case ItemTemplate.CRYSTAL_B:
						env.value += 3 * (enchant + overenchant * 2) * (isBlessed ? 1.6 : 1.0);//修復強化2倍
						break;
			...
			case POWER_ATTACK:
			{
				...
				switch(item.getTemplate().getGrade().getCrystalId())
				{
					...
					case ItemTemplate.CRYSTAL_S:
						if(isBow)
							env.value += 10 * (enchant + overenchant * 4);//修復強化S級4倍
						else if(isCrossbow)
							env.value += 7 * (enchant + overenchant * 4);//修復強化S級4倍
						else if(isSword)
							env.value += 6 * (enchant + overenchant * 4);//修復強化S級4倍
						else
							env.value += 5 * (enchant + overenchant * 4);//修復強化S級4倍
						break;
					case ItemTemplate.CRYSTAL_A:
						if(isBow)
							env.value += 8 * (enchant + overenchant * 2);//修復強化2倍
						else if(isSword)
							env.value += 5 * (enchant + overenchant * 2);//修復強化2倍
						else
							env.value += 4 * (enchant + overenchant * 2);//修復強化2倍
						break;
					case ItemTemplate.CRYSTAL_B:
						if(isBow)
							env.value += 8 * (enchant + overenchant * 2);//修復強化2倍

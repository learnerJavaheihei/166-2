...
public class EffectRestoreMP extends EffectRestore
{
	...
	{
		double newMp = effected.getCurrentMp() + power;
		newMp = Math.max(0, Math.min(newMp, effected.getMaxMp() / 100. * effected.getStat().calc(Stats.MP_LIMIT, null, null)));
		newMp = Math.max(0, newMp - effected.getCurrentMp());
		newMp = Math.min(effected.getMaxMp() - effected.getCurrentMp(), newMp);
		return (int) newMp;
	}

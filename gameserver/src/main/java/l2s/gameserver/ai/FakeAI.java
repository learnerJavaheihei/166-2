...
import l2s.gameserver.templates.fakeplayer.actions.*;
 ...
 * @author Bonux
**/
// TODO: Добавить рендомный диапазон уровней для фарм зон.
public class FakeAI extends PlayerAI implements OnDeathListener, OnLevelChangeListener, OnTeleportListener, OnPlayerChatMessageReceive
{
	...
	private static final int GIVE_CONSUMABLE_ITEMS_DELAY = 600000; // 10 минут.
	...
	private long _lastGiveConsumableItemsTime = 0L;
	...
	public boolean performFarm()
	{
		...
		{
			List<FarmZoneTemplate> availableFarmZones = new ArrayList<FarmZoneTemplate>();
			loop: for(FarmZoneTemplate f : _aiTemplate.getFarmZones()) // Выбираем подходящие зоны, в которых уже находимся.
			{
				if(!f.checkCondition(player))
					continue;
	...
	private void farm()
	{
		...
		if((_lastAttackTime + ATTACK_WAIT_DELAY) < System.currentTimeMillis())
		{
			...
			Location loc = npc != null ? npc.getLoc() : getRandomLoc(_currentFarmZone.getZoneTemplates(), player.getGeoIndex(), player.isFlying());
		...
		if(isInside(_currentFarmZone.getZoneTemplates(), loc.x, loc.y, loc.z) && player.getMovement().moveToLocation(Location.findAroundPosition(loc, 0, player.getGeoIndex()), 0, true, 50))
	...
	private void attack(Creature target)
	{
		...
		/*if(Rnd.chance(1))
		{
			...
			player.abortAttack(true, false);
			player.abortCast(true, false);
			player.getMovement().moveToLocation(Location.findAroundPosition(player.getLoc(), 500, 1000, player.getGeoIndex()), 250, true);
			return;
		}*/
	...
	private static Location getRandomLoc(List<ZoneTemplate> zoneTemplates, int geoIndex, boolean fly)
	{
		ZoneTemplate zoneTemplate = Rnd.get(zoneTemplates);
		if(zoneTemplate != null)
			return zoneTemplate.getTerritory().getRandomLoc(geoIndex,fly);
		return new Location();
	}

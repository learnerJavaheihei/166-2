...
public class DefaultAI extends NpcAI
{
	...
	{
		Task task = new Task();
		task.type = skillEntry == null ? TaskType.ATTACK : (skillEntry.getTemplate().isOffensive() ? TaskType.CAST : TaskType.BUFF);
		task.target = target.getRef();
		task.skillEntry = skillEntry;
		task.weight = weight;
		if(skillEntry != null)
	...
	{
		Task task = new Task();
		task.type = skillEntry.getTemplate().isOffensive() ? TaskType.CAST : TaskType.BUFF;
		task.target = target.getRef();
		task.skillEntry = skillEntry;
		task.p1 = p1;
		task.p2 = p2;
	...
	protected void notifyFriendsOnAttack(Creature attacker, Skill skill, int damage)
	{
		...
		if((System.currentTimeMillis() - _lastFactionNotifyTime) > _minFactionNotifyInterval)
		{
			...
			//20200105有靜行術之類的特效就不互助了。
			if(attacker.isContainsAbnormalEffect(AbnormalEffect.STEALTH))
			{
				return;
			}	

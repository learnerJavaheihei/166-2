...
public class NpcAI extends CharacterAI
{
	...
	{
		super.notifyFriendsOnAttack(attacker, skill, damage);

		if(damage <= 0)
			return;

		for(NpcInstance minion : _exPrivates)
			minion.getAI().notifyEvent(CtrlEvent.EVT_PARTY_ATTACKED, getActor(), attacker, damage);
	}

	@Override
	protected void notifyFriendsOnDie(Creature killer)
	{
		super.notifyFriendsOnDie(killer);

		for(NpcInstance minion : _exPrivates)
			minion.getAI().notifyEvent(CtrlEvent.EVT_PARTY_DIED, getActor(), killer);
	}*/

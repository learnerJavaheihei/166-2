...
public final class SkillAcquireParser extends StatParser<SkillAcquireHolder>
{
	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		//認證技能--
		for(Iterator<Element> iterator = rootElement.elementIterator("adventurer_skill_tree"); iterator.hasNext();)
			getHolder().addAllAdventurerLearns(parseSkillLearn(iterator.next()));

		for(Iterator<Element> iterator = rootElement.elementIterator("forgottenPower_skill_tree"); iterator.hasNext();)
			getHolder().addAllForgottenPowerLearns(parseSkillLearn(iterator.next()));
		//--遺忘之力

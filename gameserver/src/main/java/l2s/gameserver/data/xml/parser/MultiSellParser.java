	...
	protected MultiSellEntry parseEntry(Element n, int multiSellId)
	{
		...
		for(Iterator<Element> iterator = n.elementIterator(); iterator.hasNext();)
		{
			...
			if("ingredient".equalsIgnoreCase(d.getName()))
			{
				...
				int enchanted = 0;//multisell支持enchant--
				if (d.attributeValue("enchant") != null)
				{
					enchanted = Integer.parseInt(d.attributeValue("enchant"));
				}
				if(enchanted > 0)
				{
					entry.addIngredient(new MultiSellIngredient(id, count, 0, -1, -1, enchanted, 0));//multisell支持bouns
				}
				else
				{
					entry.addIngredient(new MultiSellIngredient(id, count));//原始就有
				}//--multisell支持enchant
			}
			else if("production".equalsIgnoreCase(d.getName()))
			{
				...
				int bouns = d.attributeValue("bouns") == null ? 0 : Integer.parseInt(d.attributeValue("bouns"));//multisell支持bouns
				int flags = 0;
				...
				if(flagsArray != null)
				{
					for(String flag : flagsArray)
					{
						switch(flag)
						{
							case "FLAG_NO_DROP": flags |= ItemInstance.FLAG_NO_DROP; break;
							case "FLAG_NO_TRADE": flags |= ItemInstance.FLAG_NO_TRADE; break;
							case "FLAG_NO_TRANSFER": flags |= ItemInstance.FLAG_NO_TRANSFER; break;
							case "FLAG_NO_CRYSTALLIZE": flags |= ItemInstance.FLAG_NO_CRYSTALLIZE; break;
						}
					}
				}
				...
				entry.addProduct(new MultiSellIngredient(id, count, chance, flags, durablity, enchant, bouns));//multisell支持bouns
			}
		}

...
public abstract class StatParser<H extends AbstractHolder> extends AbstractParser<H>
{
	...
	protected Condition parsePlayerCondition(Element element, int... arg)
	{
		...
		for(Iterator<Attribute> iterator = element.attributeIterator(); iterator.hasNext();)
		{
			...
			else if(name.equalsIgnoreCase("Charges"))//新增鬥力需求--
				cond = joinAnd(cond, new ConditionPlayerCharges(Integer.parseInt(value)));//--新增鬥力需求
	...
	protected void parseFor(Element forElement, StatTemplate template, int... arg)
	{
		for(Iterator<Element> iterator = forElement.elementIterator(); iterator.hasNext();)
		{
			...
			else if(elementName.startsWith("p_"))
			{
				...
				else if(elementName.toLowerCase().startsWith("p_elemental_"))
				{
					String elementalElementName = parseString(element.attributeValue("element"), arg);
					Stats stat = Stats.valueOfXml(elementName.replaceFirst("^p_", elementalElementName + "_"));
					if(elementName.equals("p_elemental_exp_rate"))
						attachNewFunc(element, template, stat, StatModifierType.PER, arg);
					else
						attachNewFunc(element, template, stat, arg);
				}
				else
				{
	...
	protected void attachNewFunc(Element n, StatTemplate template, Stats stat, StatModifierType type, int... arg)
	{
		Condition applyCond = parseFirstCond(n, arg);
		double value = parseNumber(n.attributeValue("value"), arg).doubleValue();
		template.attachFunc(new FuncTemplate(applyCond, stat, value, type));
	}

	protected void attachNewFunc(Element n, StatTemplate template, Stats stat, int... arg)
	{
		StatModifierType type = StatModifierType.valueOf(parseString(n.attributeValue("type"), arg).toUpperCase());
		attachNewFunc(n, template, stat, type, arg);
	}

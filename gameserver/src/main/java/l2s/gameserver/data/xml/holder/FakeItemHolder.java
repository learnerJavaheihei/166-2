...
public class FakeItemHolder extends AbstractHolder
{
	...
	private final Map<ItemGrade, Map<ArmorTemplate.ArmorType, IntList>> armors = new HashMap<>();
	...
	public void addArmors(ItemGrade grade, Map<ArmorTemplate.ArmorType, IntList> map)
	{
			...
			case "Armor":
			{
				try
				{
					...
					if(classWeaponAndArmor == null)
						return Containers.EMPTY_INT_LIST;
					Map<ArmorTemplate.ArmorType, IntList> packs = armors.get(grade);
					if(packs == null || packs.isEmpty())
						return Containers.EMPTY_INT_LIST;
					result = packs.get(classWeaponAndArmor.getRandomArmorType());
				}
				catch(Exception e)
				{

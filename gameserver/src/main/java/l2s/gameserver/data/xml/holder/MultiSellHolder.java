				...
				for(MultiSellIngredient ingredient : ingridients)
				{
					...
					if(ingredient.getItemId() <= 0 || nokey || template.isEquipment())
					{
						...
						for(final ItemInstance item : items)
						{
							...
							{
								if(item.isEquipped())//multisell支持enchant
								{
									break;
								}//--multisell支持enchant
								...
								if(enchant)//multisell支持bouns--
								{
									if(item.getEnchantLevel() < ingredient.getItemEnchant()) // Некоторые мультиселлы требуют заточки 原始就有
										continue;
								}else
								{
								...
								for(MultiSellIngredient p : ent.getProduction())
								{
									...
									else if(p.getItemEnchant() > 0)//multisell支持enchant--
									{
										p.setItemEnchant(p.getItemEnchant());
									}//--multisell支持enchant
								...
								for(MultiSellIngredient ig : ingridients)
								{
									...
									else if(ig.getItemEnchant() > 0)//multisell支持enchant--
									{
										ig.setItemEnchant(ig.getItemEnchant());
									}//--multisell支持enchant

...
public class BotSkillStrategy
{
	...
	public boolean useMe(Player actor, MonsterInstance target)
	{
		...
		if(skillEntry == null)
			return false;
		// 矮子 字体变化只能用一次
		if (skillEntry.getTemplate().getId() == 254 || skillEntry.getTemplate().getId() == 302) {
			// 设为一次使用
			setOneTime(true);
		}
	...
	public String toString()
	{
		...
		return String.valueOf(SkillHolder.getInstance().getSkill(this.getSkillId(), 1).getName()) + (this.isCheckOnTargetHp() ? "[目标HP" : "[自身HP") + this.getHpPercentCheck() + "%]" + (this.isOneTime() ? "[一次]" : "[重复]");
	}

	public String toTableTd()
	{
